---
title: Server
description: Interact with agence server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `agence serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an agence client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `agence.local` |
...
Set `AGENCE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `agence`, or set `AGENCE_SERVER_USERNAME` to override it. This applies to both `agence serve` and `agence web`.
...
When you run `agence` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the agence server to interact with agence programmatically.
:::

This architecture lets agence support multiple clients and allows you to interact with agence programmatically.

You can run `agence serve` to start a standalone server. If you have the
agence TUI running, `agence serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Agence [IDE](/docs/ide) plugins.
...
The agence server exposes the following APIs.
