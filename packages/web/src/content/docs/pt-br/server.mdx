---
title: Servidor
description: Interaja com o servidor agence via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `agence serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente agence pode usar.
...
```bash
agence serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `agence.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
agence serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `AGENCE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `agence`, ou defina `AGENCE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `agence serve` quanto ao `agence web`.

```bash
AGENCE_SERVER_PASSWORD=your-password agence serve
```
...
Quando você executa `agence`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor agence para interagir com o agence programaticamente.
:::

Essa arquitetura permite que o agence suporte múltiplos clientes e permite que você interaja com o agence programaticamente.

Você pode executar `agence serve` para iniciar um servidor autônomo. Se você tiver o TUI do agence em execução, `agence serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do agence [IDE](/docs/ide).
...
O servidor agence expõe as seguintes APIs.
