---
title: Plugins
description: Escreva seus próprios plugins para estender o agence.
---

Plugins permitem que você estenda o agence conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do agence.
...
O agence executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do agence para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o agence.
...
:::note
Se você estiver usando o aplicativo desktop agence, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o agence de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao agence:
...
O helper `tool` cria uma ferramenta personalizada que o agence pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o agence junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://github.com/David2024patton/agence/docs/sdk) para detalhes.
