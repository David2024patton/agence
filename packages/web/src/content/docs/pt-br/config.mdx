---
title: Configuração
description: Usando a configuração JSON do agence.
---

Você pode configurar o agence usando um arquivo de configuração JSON.

---

## Formato

O agence suporta os formatos **JSON** e **JSONC** (JSON com Comentários).
...
1. **Configuração remota** (de `.well-known/agence`) - padrões organizacionais
2. **Configuração global** (`~/.config/opencode/opencode.json`) - preferências do usuário
3. **Configuração personalizada** (`AGENCE_CONFIG` var de ambiente) - substituições personalizadas
...
6. **Configuração inline** (`AGENCE_CONFIG_CONTENT` var de ambiente) - substituições em tempo de execução
...
:::note
Os diretórios `.opencode` e `~/.config/agence` usam **nomes no plural** para subdiretórios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, e `themes/`. Nomes no singular (por exemplo, `agent/`) também são suportados para compatibilidade retroativa.
:::
...
As organizações podem fornecer configuração padrão através do endpoint `.well-known/agence`. Isso é buscado automaticamente quando você se autentica com um provedor que o suporta.
...
Coloque sua configuração global do agence em `~/.config/opencode/opencode.json`. Use a configuração global para preferências de todo o usuário, como provedores, modelos e permissões.
...
Quando o agence é iniciado, ele procura um arquivo de configuração no diretório atual ou sobe até o diretório Git mais próximo.
...
Especifique um caminho de arquivo de configuração personalizado usando a variável de ambiente `AGENCE_CONFIG`.
...
Especifique um diretório de configuração personalizado usando a variável de ambiente `AGENCE_CONFIG_DIR`. Este diretório será pesquisado por agentes, comandos, modos e plugins, assim como o diretório padrão `.opencode`, e deve seguir a mesma estrutura.
...
O esquema de configuração do servidor/tempo de execução é definido em [**`agence.ai/config.json`**](https://github.com/David2024patton/agence/config.json).

A configuração do TUI usa [**`agence.ai/tui.json`**](https://github.com/David2024patton/agence/tui.json).
...
Use `AGENCE_TUI_CONFIG` para apontar para um arquivo de configuração TUI personalizado.
...
Você pode configurar as configurações do servidor para os comandos `agence serve` e `agence web` através da opção `server`.
...
- `mdns` - Habilitar descoberta de serviço mDNS. Isso permite que outros dispositivos na rede descubram seu servidor agence.
- `mdnsDomain` - Nome de domínio personalizado para o serviço mDNS. O padrão é `agence.local`. Útil para executar várias instâncias na mesma rede.
...
Você pode configurar os provedores e modelos que deseja usar em sua configuração do agence através das opções `provider`, `model` e `small_model`.
...
A opção `small_model` configura um modelo separado para tarefas leves, como geração de títulos. Por padrão, o agence tenta usar um modelo mais barato se um estiver disponível do seu provedor, caso contrário, ele recua para seu modelo principal.
...
O agente padrão deve ser um agente primário (não um subagente). Isso pode ser um agente embutido como `"build"` ou `"plan"`, ou um [agente personalizado](/docs/agents) que você definiu. Se o agente especificado não existir ou for um subagente, o agence recuará para `"build"` com um aviso.

Essa configuração se aplica a todas as interfaces: TUI, CLI (`agence run`), aplicativo desktop e GitHub Action.
...
O agence fará o download automaticamente de quaisquer novas atualizações quando for iniciado. Você pode desabilitar isso com a opção `autoupdate`.
...
Por padrão, o agence **permite todas as operações** sem exigir aprovação explícita. Você pode mudar isso usando a opção `permission`.
...
[Plugins](/docs/plugins) estendem o agence com ferramentas, hooks e integrações personalizadas.
...
Isso é útil quando você deseja restringir o agence para usar apenas provedores específicos, em vez de desabilitá-los um a um.
