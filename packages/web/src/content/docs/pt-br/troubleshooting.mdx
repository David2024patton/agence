...
Para depurar problemas com o agence, comece verificando os logs e os dados locais que ele armazena no disco.
...
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\agence\log`
...
Você pode definir o nível de log com a opção de linha de comando `--log-level` para obter informações de depuração mais detalhadas. Por exemplo, `agence --log-level DEBUG`.

---

## Armazenamento

O agence armazena dados de sessão e outros dados do aplicativo no disco em:

- **macOS/Linux**: `~/.local/share/opencode/`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\agence`
...
O agence Desktop executa um servidor agence local (o sidecar `opencode-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
- Apenas macOS: menu `Agence` -> **Recarregar Webview** (ajuda se a interface estiver em branco/congelada).
...
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\agence\opencode.jsonc`
...
O agence também pode carregar plugins locais do disco. Mova-os temporariamente para fora do caminho (ou renomeie a pasta) e reinicie o aplicativo de desktop:

- **Plugins globais**
  - **macOS/Linux**: `~/.config/opencode/plugins/`
  - **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\agence\plugins`
...
Se desativar plugins não ajudar (ou se a instalação de um plugin estiver travada), limpe o cache para que o agence possa reconstruí-lo.

1. Saia completamente do agence Desktop.
2. Exclua o diretório de cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> cole `~/.cache/agence`
- **Linux**: exclua `~/.cache/agence` (ou execute `rm -rf ~/.cache/agence`)
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.cache\agence`

3. Reinicie o agence Desktop.

---

### Corrigindo problemas de conexão com o servidor

O agence Desktop pode iniciar seu próprio servidor local (padrão) ou conectar-se a uma URL de servidor que você configurou.
...
Se você tiver `AGENCE_PORT` definido em seu ambiente, o aplicativo de desktop tentará usar essa porta para o servidor local.

- Desfaça `AGENCE_PORT` (ou escolha uma porta livre) e reinicie.
...
No Windows, o agence Desktop requer o **WebView2 Runtime** do Microsoft Edge. Se o aplicativo abrir em uma janela em branco ou não iniciar, instale/atualize o WebView2 e tente novamente.

---

### Windows: Problemas gerais de desempenho

Se você estiver enfrentando desempenho lento, problemas de acesso a arquivos ou problemas no terminal no Windows, tente usar [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). O WSL fornece um ambiente Linux que funciona de forma mais integrada com os recursos do agence.

---

### Notificações não aparecendo

O agence Desktop só mostra notificações do sistema quando:

- as notificações estão habilitadas para o agence nas configurações do seu sistema operacional, e
...
1. Saia do agence Desktop.
2. Encontre e exclua estes arquivos (eles estão no diretório de dados do aplicativo agence Desktop):

- `agence.settings.dat` (URL do servidor padrão do desktop)
- `agence.global.dat` e `agence.workspace.*.dat` (estado da interface como servidores/projetos recentes)
...
Se você estiver enfrentando problemas com o agence:
...
3. Certifique-se de que você tem a versão mais recente com `agence upgrade`
...
Para descobrir quais modelos você tem acesso, execute `agence models`
...
2. Se o problema persistir, tente limpar sua configuração armazenada:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.local\share\agence`
...
Se você encontrar erros de chamada de API, isso pode ser devido a pacotes de provedores desatualizados. O agence instala dinamicamente pacotes de provedores (OpenAI, Anthropic, Google, etc.) conforme necessário e os armazena em cache localmente.
...
1. Limpe o cache do pacote de provedores:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.cache\agence`

2. Reinicie o agence para reinstalar os pacotes de provedores mais recentes

Isso forçará o agence a baixar as versões mais recentes dos pacotes de provedores, o que muitas vezes resolve problemas de compatibilidade com parâmetros de modelo e alterações na API.
...
O agence detectará se você está usando Wayland e preferirá `wl-clipboard`, caso contrário, tentará encontrar ferramentas de área de transferência na ordem: `xclip` e `xsel`.
