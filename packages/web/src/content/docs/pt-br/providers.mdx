---
title: Provedores
description: Usando qualquer provedor LLM no agence.
---

import config from "../../../../config.mjs"
export const console = config.console

agence usa o [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) para suportar **75+ provedores LLM** e suporta a execução de modelos locais.

Para adicionar um provedor, você precisa:

1. Adicionar as chaves da API para o provedor usando o comando `/connect`.
2. Configurar o provedor na sua configuração do agence.
...
Você pode personalizar os provedores através da seção `provider` na sua configuração do agence.
...
Agence Zen é uma lista de modelos fornecidos pela equipe do agence que foram testados e verificados para funcionar bem com o agence. [Saiba mais](/docs/zen).

:::tip
Se você é novo, recomendamos começar com o Agence Zen.
:::

1. Execute o comando `/connect` no TUI, selecione `Agence Zen` e acesse [agence.ai/zen](https://github.com/David2024patton/agence/zen).
...
Funciona como qualquer outro provedor no agence e é completamente opcional.

---

## Agence Go

Agence Go é um plano de assinatura de baixo custo que fornece acesso confiável a modelos de codificação abertos populares fornecidos pela equipe do agence que foram
testados e verificados para funcionar bem com o agence.

1. Execute o comando `/connect` no TUI, selecione `Agence Go` e acesse [agence.ai/zen](https://github.com/David2024patton/agence/zen).
...
Para usar o Amazon Bedrock com o agence:
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   ***
   ...
   Defina uma dessas variáveis de ambiente ao executar o agence:
...
:::info
Usar sua assinatura Claude Pro/Max no agence não é oficialmente suportado pela [Anthropic](https://anthropic.com).
:::
...
Você pode configurar o agence para usar modelos locais através do [Atomic Chat](https://atomic.chat), um aplicativo de desktop que executa LLMs locais por trás de um servidor API compatível com OpenAI (endpoint padrão `http://127.0.0.1:1337/v1`).
...
- `name` é o nome de exibição do provedor na interface.
- `options.baseURL` é o endpoint do servidor local. Altere host e porta para corresponder à sua configuração do Atomic Chat.
- `models` é um mapa de IDs de modelos para seus nomes de exibição. Cada ID deve corresponder ao `id` retornado por `GET /v1/models` — execute `curl http://127.0.0.1:1337/v1/models` para listar os IDs atualmente carregados no Atomic Chat.

:::tip
Se as chamadas de ferramentas não estiverem funcionando bem, escolha um modelo carregado com bom suporte a tool calling (por exemplo, uma variante Qwen-Coder ou DeepSeek-Coder).
:::
...
2. Vá para [Azure AI Foundry](https://ai.azure.com/) e implante um modelo.
   ...
   :::note
   O nome da implantação deve corresponder ao nome do modelo para que o agence funcione corretamente.
   :::
...
5. Execute o comando `/models` para selecionar um modelo.
   ...
   Você também pode adicionar modelos através da sua configuração do agence.
...
armazenar o token no armazenamento de autenticação do agence.
:::

##### GitLab Auto-Hospedado

:::note[nota de conformidade]
agence usa um pequeno modelo para algumas tarefas de AI, como gerar o título da sessão.
Ele está configurado para usar gpt-5-nano por padrão, hospedado pela Zen. Para bloquear o agence
...
Para usar sua assinatura do GitHub Copilot com o agence:
...
Para usar o Google Vertex AI com o agence:
...
2. Defina as variáveis de ambiente necessárias:
   ...
   Defina-as ao executar o agence.
...
Caso você veja um recurso ou modelo do Helicone que não está configurado automaticamente através do agence, você sempre pode configurá-lo você mesmo.
...
O recurso [Sessões](https://docs.helicone.ai/features/sessions) do Helicone permite que você agrupe solicitações LLM relacionadas. Use o plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) para registrar automaticamente cada conversa do agence como uma sessão no Helicone.
...
O plugin injeta os cabeçalhos `Helicone-Session-Id` e `Helicone-Session-Name` em suas solicitações. Na página de Sessões do Helicone, você verá cada conversa do agence listada como uma sessão separada.
...
Você pode configurar o agence para usar modelos locais através da utilidade llama-server do [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Você pode configurar o agence para usar modelos locais através do LM Studio.
...
Você pode configurar o agence para usar modelos locais através do Ollama.

:::tip
Ollama pode se configurar automaticamente para o agence. Consulte a [documentação de integração do Ollama](https://docs.ollama.com/integrations/agence) para detalhes.
:::
...
Para usar o Ollama Cloud com o agence:
...
3. Copie a chave da API para uso no agence.
...
6. **Importante**: Antes de usar modelos em nuvem no agence, você deve puxar as informações do modelo localmente:
...
Agence Zen é uma lista de modelos testados e verificados fornecidos pela equipe do agence. [Saiba mais](/docs/zen).

1. Faça login em **<a href={console}>Agence Zen</a>** e clique em **Criar Chave da API**.

2. Execute o comando `/connect` e procure por **Agence Zen**.

   ```txt
   /connect
   ```

3. Insira sua chave da API do agence.
...
4. Muitos modelos do OpenRouter são pré-carregados por padrão, execute o comando `/models` para selecionar o que você deseja.
   ...
   Você também pode adicionar modelos adicionais através da sua configuração do agence.
...
5. Você também pode personalizá-los através da sua configuração do agence. Aqui está um exemplo de especificar um provedor
...
Para usar [APIs Generativas Scaleway](https://www.scaleway.com/en/docs/generative-apis/) com o agence:
...
Você também pode personalizar modelos através da sua configuração do agence. Aqui está um exemplo de especificar a ordem de roteamento do provedor.
...
:::tip
Você pode usar qualquer provedor compatível com OpenAI com o agence. A maioria dos provedores de AI modernos oferece APIs compatíveis com OpenAI.
:::
...
Os campos `limit` permitem que o agence entenda quanto contexto você tem disponível. Provedores padrão puxam isso automaticamente do models.dev.
...
1. **Verifique a configuração de autenticação**: Execute `agence auth list` para ver se as credenciais
...
2. Para provedores personalizados, verifique a configuração do agence e:
   - Certifique-se de que o ID do provedor usado no comando `/connect` corresponda ao ID na sua configuração do agence.
