...
O Agence Go é uma assinatura de baixo custo — **US$ 5 no seu primeiro mês**, depois **US$ 10/mês** — que oferece acesso confiável a modelos abertos de programação populares.

O Go funciona como qualquer outro provedor no Agence. Você assina o Agence Go e
...
O Agence Go lhe dá acesso a esses modelos por **US$ 5 no seu primeiro mês**, depois **US$ 10/mês**.

---

## Como funciona

O Agence Go funciona como qualquer outro provedor no Agence.

1. Você entra no **<a href={console}>Agence Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `Agence Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o Agence Go.
:::
...
O Agence Go inclui os seguintes limites:
...
O [ID do modelo](/docs/config/#models) na sua configuração do Agence
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o Agence também.
