...
Você pode adicionar ferramentas externas ao agence usando o _Model Context Protocol_, ou MCP. O agence suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do agence](https://github.com/David2024patton/agence/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
As organizações podem fornecer servidores MCP padrão através de seu endpoint `.well-known/agence`. Esses servidores podem estar desabilitados por padrão, permitindo que os usuários optem pelos que precisam.
...
O agence lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o agence irá:
...
Se o servidor requer autenticação, o agence solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `agence mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o agence armazenará os tokens de forma segura em `~/.local/share/opencode/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no agence, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do agence como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o agence à sua conta do Sentry.
