...
To debug issues with Agence, start by checking the logs and local data it stores on disk.
...
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.local\share\agence\log`

Log files are named with timestamps (e.g., `2025-01-09T123456.log`) and the most recent 10 log files are kept.

You can set the log level with the `--log-level` command-line option to get more detailed debug information. For example, `agence --log-level DEBUG`.

---

## Storage

agence stores session data and other application data on disk at:

- **macOS/Linux**: `~/.local/share/opencode/`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.local\share\agence`
...
Agence Desktop runs a local Agence server (the `opencode-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.

### Quick checks

- Fully quit and relaunch the app.
- If the app shows an error screen, click **Restart** and copy the error details.
- macOS only: `Agence` menu -> **Reload Webview** (helps if the UI is blank/frozen).
...
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\agence\opencode.jsonc`
...
Agence can also load local plugins from disk. Temporarily move these out of the way (or rename the folder) and restart the desktop app:

- **Global plugins**
  - **macOS/Linux**: `~/.config/opencode/plugins/`
  - **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\agence\plugins`
...
If disabling plugins doesn't help (or a plugin install is stuck), clear the cache so Agence can rebuild it.

1. Quit Agence Desktop completely.
2. Delete the cache directory:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/agence`
- **Linux**: delete `~/.cache/agence` (or run `rm -rf ~/.cache/agence`)
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.cache\agence`

3. Restart Agence Desktop.

---

### Fix server connection issues

Agence Desktop can either start its own local server (default) or connect to a server URL you configured.
...
If you have `AGENCE_PORT` set in your environment, the desktop app will try to use that port for the local server.

- Unset `AGENCE_PORT` (or pick a free port) and restart.
...
On Windows, Agence Desktop requires the Microsoft Edge **WebView2 Runtime**. If the app opens to a blank window or won't start, install/update WebView2 and try again.

---

### Windows: General performance issues

If you're experiencing slow performance, file access issues, or terminal problems on Windows, try using [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL provides a Linux environment that works more seamlessly with Agence's features.

---

### Notifications not showing

Agence Desktop only shows system notifications when:

- notifications are enabled for Agence in your OS settings, and
...
1. Quit Agence Desktop.
2. Find and delete these files (they live in the Agence Desktop app data directory):

- `agence.settings.dat` (desktop default server URL)
- `agence.global.dat` and `agence.workspace.*.dat` (UI state like recent servers/projects)
...
If you're experiencing issues with Agence:
...
3. Ensure you have the latest version with `agence upgrade`
...
To figure out what models you have access to, run `agence models`
...
2. If the issue persists, try clearing your stored configuration:
   ...
   On Windows, press `WIN+R` and delete: `%USERPROFILE%\.local\share\agence`
...
If you encounter API call errors, this may be due to outdated provider packages. agence dynamically installs provider packages (OpenAI, Anthropic, Google, etc.) as needed and caches them locally.
...
1. Clear the provider package cache:
   ...
   On Windows, press `WIN+R` and delete: `%USERPROFILE%\.cache\agence`

2. Restart agence to reinstall the latest provider packages

This will force agence to download the most recent versions of provider packages, which often resolves compatibility issues with model parameters and API changes.
...
agence will detect if you're using Wayland and prefer `wl-clipboard`, otherwise it will try to find clipboard tools in order of: `xclip` and `xsel`.
