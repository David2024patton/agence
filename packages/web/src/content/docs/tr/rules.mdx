---
title: Kurallar
description: agence için özel talimatları ayarlayın.
---

Bir `AGENTS.md` dosyası oluşturarak agence için özel talimatlar verebilirsiniz. Bu, Cursor rules yaklaşımına benzer. Özel projeniz için davranışını özelleştirmek amacıyla LLM'in içeriğine dahil edilecek talimatları içerir.

---

## Başlat

Yeni bir `AGENTS.md` dosyası oluşturmak için `/init` komutunu agence'da çalıştırabilirsiniz.

:::tip
Projenizin `AGENTS.md` dosyasını Git'e göndermelisiniz.
:::

Bu, projenin neyle ilgili olduğunu anlamak için projenizi ve tüm içeriğini tarayacak ve onunla bir `AGENTS.md` dosyası oluşturacaktır. Bu, agence'un projede daha iyi gezinmesine yardımcı olur.
...
agence ayrıca `AGENTS.md` dosyasının birden fazla konumdan okunmasını da destekler. Ve bu farklı amaçlara hizmet ediyor.
...
Ayrıca `~/.config/opencode/AGENTS.md` dosyasında genel kurallara sahip olabilirsiniz. Bu, tüm agence oturumlarına uygulanır.
...
agence, Claude Code'dan geçiş yapan kullanıcılar için yedek olarak Claude Code'un dosya kurallarını destekler:
...
`AGENTS.md`'nizde açık talimatlar sağlayarak agence'u harici dosyaları okumayı öğretebilirsiniz. İşte pratik bir örnek:
...
- agence'un dosyaları yalnızca belirli bir görev için gerektiğinde yüklediğinden emin olun
