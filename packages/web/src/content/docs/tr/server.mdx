---
title: Sunucu
description: agence sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`agence serve` komutu, agence istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `agence.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `AGENCE_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `agence` değeridir; değiştirmek isterseniz `AGENCE_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `agence serve` hem de `agence web` için geçerlidir.
...
`agence` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
agence ile programatik etkileşim için agence sunucusunu kullanın.
:::

Bu mimari, agence'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`agence serve` ile bağımsız bir sunucu başlatabilirsiniz. agence TUI açıksa, `agence serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum agence [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
agence sunucusu aşağıdaki API'leri sunar.
