...
agence ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\agence\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `agence --log-level DEBUG`.

---

## Depolama

agence, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/opencode/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\agence` yapistirin
...
agence Desktop arka planda yerel bir agence sunucusu (`opencode-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `agence` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\agence\opencode.jsonc` yapistirin
...
agence diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/opencode/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\agence\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), agence'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. agence Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/agence`
- **Linux**: `~/.cache/agence` dizinini silin (veya `rm -rf ~/.cache/agence` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\agence` yapistirin

3. agence Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

agence Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
Ortamda `AGENCE_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `AGENCE_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
...
Windows'ta agence Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, agence ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

agence Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda agence icin bildirimler etkinse
...
1. agence Desktop'u kapatin
2. Su dosyalari bulun ve silin (agence Desktop uygulama veri dizininde yer alirlar):

- `agence.settings.dat` (desktop varsayilan sunucu URL'si)
- `agence.global.dat` ve `agence.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
agence ile ilgili bir sorun yasiyorsaniz:
...
3. `agence upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `agence models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\agence`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. agence, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\agence`

2. En guncel provider paketlerini yeniden kurmak icin agence'u yeniden baslatin

Bu, agence'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
agence, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
