...
Agence Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, Agence'daki diğer sağlayıcılar gibi çalışır. Agence Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Agence'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Agence Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Agence Go, Agence'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Agence Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Agence Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Agence Go'ya abone olabilir.
:::
...
Agence Go aşağıdaki limitleri içerir:
...
Agence yapılandırmanızdaki [model id](/docs/config/#models) formatı `opencode-go/<model-id>` şeklindedir. Örneğin, Kimi K2.6 için yapılandırmanızda `opencode-go/kimi-k2.6` kullanmalısınız.
...
Agence Go'yu şu amaçlarla oluşturduk:
...
4. Agence ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
