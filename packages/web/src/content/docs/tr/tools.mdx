...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. agence yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
agence içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `AGENCE_EXPERIMENTAL_LSP_TOOL=true` (veya `AGENCE_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca agence provider kullanılırken veya `AGENCE_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

agence başlatırken etkinleştirmek için:
