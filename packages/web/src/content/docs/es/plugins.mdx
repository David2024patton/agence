---
title: Complementos
description: Escriba sus propios complementos para extender Agence.
---

Los complementos le permiten extender Agence al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Agence.
...
Agence ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.
...
- `client`: Un cliente SDK agence para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar agence.
  ...
  return {
    event: async ({ event }) => {
      ...
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "agence"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Agence, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que agence lea archivos `.env`:
...
Los complementos también pueden agregar herramientas personalizadas a agence:

```ts title=".opencode/plugins/custom-tools.ts"
import { type Plugin, tool } from "@agence-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que agence puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para agence junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://github.com/David2024patton/agence/docs/sdk) para obtener más detalles.
