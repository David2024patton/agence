...
Las herramientas permiten que LLM realice acciones en su código base. Agence viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Agence.
...
:::note
Esta herramienta solo está disponible cuando `AGENCE_EXPERIMENTAL_LSP_TOOL=true` (o `AGENCE_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Agence o cuando la variable de entorno `AGENCE_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Agence:

```bash
AGENCE_ENABLE_EXA=1 agence
```
