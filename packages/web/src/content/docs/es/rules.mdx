---
title: Reglas
description: Establezca instrucciones personalizadas para agence.
---

Puede proporcionar instrucciones personalizadas a agence creando un archivo `AGENTS.md`. Esto es similar a las reglas del Cursor. Contiene instrucciones que se incluirán en el contexto de LLM para personalizar su comportamiento para su proyecto específico.

---

## Inicializar

Para crear un nuevo archivo `AGENTS.md`, puede ejecutar el comando `/init` en agence.

:::tip
Debes enviar el archivo `AGENTS.md` de tu proyecto a Git.
:::

Esto escaneará su proyecto y todo su contenido para comprender de qué se trata y generar un archivo `AGENTS.md` con él. Esto ayuda a agence a navegar mejor por el proyecto.
...
agence también admite la lectura del archivo `AGENTS.md` desde múltiples ubicaciones. Y esto tiene diferentes propósitos.
...
También puedes tener reglas globales en un archivo `~/.config/opencode/AGENTS.md`. Esto se aplica en todas las sesiones agence.
...
Para los usuarios que migran desde Claude Code, Agence admite las convenciones de archivos de Claude Code como alternativas:
...
Cuando se inicia agence, busca archivos de reglas en este orden:
...
Si bien agence no analiza automáticamente las referencias de archivos en `AGENTS.md`, puedes lograr una funcionalidad similar de dos maneras:
...
Puede enseñarle a agence a leer archivos externos proporcionando instrucciones explícitas en su `AGENTS.md`. He aquí un ejemplo práctico:
...
- Asegúrese de que agence cargue archivos solo cuando sea necesario para la tarea específica
