...
Para depurar problemas con Agence, comience verificando los registros y los datos locales que almacena en el disco.
...
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\agence\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `agence --log-level DEBUG`.

---

## Almacenamiento

agence almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/opencode/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\agence`
...
Agence Desktop ejecuta un servidor Agence local (el sidecar `opencode-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `Agence` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\agence\opencode.jsonc`
...
Agence también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/opencode/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\agence\plugins`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que Agence pueda reconstruirlo.

1. Salga de Agence Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/agence`
- **Linux**: eliminar `~/.cache/agence` (o ejecutar `rm -rf ~/.cache/agence`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\agence`

3. Reinicie el escritorio Agence.

---

### Solucionar problemas de conexión del servidor

Agence Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si tiene `AGENCE_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `AGENCE_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio Agence requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de Agence.

---

### Notificaciones que no se muestran

Agence Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para Agence en la configuración de su sistema operativo, y
...
1. Salga del escritorio Agence.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio Agence):

- `agence.settings.dat` (URL del servidor predeterminado de escritorio)
- `agence.global.dat` y `agence.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
2. **Únete a nuestro Discord**
   ...
   [**agence.ai/discord**](https://github.com/David2024patton/agence/discord)
...
3. Asegúrese de tener la última versión con `agence upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `agence models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   ```bash
   rm -rf ~/.local/share/agence
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\agence`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. agence instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\agence`

2. Reinicie agence para reinstalar los paquetes del proveedor más recientes.

Esto obligará a agence a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
agence detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
