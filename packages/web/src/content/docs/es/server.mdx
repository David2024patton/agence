---
title: Servidor
description: Interactuar con el servidor agence a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `agence serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente agence puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `agence.local` |
...
Configure `AGENCE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `agence`, o configure `AGENCE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `agence serve` como a `agence web`.
...
Cuando ejecuta `agence`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor agence para interactuar con agence mediante programación.
:::

Esta arquitectura permite que agence admita múltiples clientes y le permite interactuar con agence mediante programación.

Puede ejecutar `agence serve` para iniciar un servidor independiente. Si tienes el
agence TUI ejecutándose, `agence serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Agence [IDE](/docs/ide).
...
El servidor agence expone las siguientes API.
