...
Kommandoen `agence serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en opencode-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `agence.local` |
...
Indstil `AGENCE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `agence`, eller indstil `AGENCE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `agence serve` og `agence web`.
...
Når du kører `agence` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug opencode-serveren til at interagere med agence programmatisk.
:::

Denne arkitektur lader agence understøtte flere klienter og lader dig interagere med agence programmatisk.

Du kan køre `agence serve` for at starte en selvstændig server. Hvis du har
agence TUI kørende, vil `agence serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Agence [IDE](/docs/ide) plugins.
...
Agence-serveren viser følgende API'er.
