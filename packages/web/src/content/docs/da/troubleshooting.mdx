...
For at fejlfinde problemer med Agence, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\agence\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `agence --log-level DEBUG`.

---

## Lagring

agence gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/opencode/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\agence`
...
Agence Desktop kører en lokal Agence-server (`opencode-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Agence`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\agence\opencode.jsonc`
...
Agence kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/opencode/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\agence\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Agence kan genopbygge den.

1. Afslut Agence Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/agence`
- **Linux**: slet `~/.cache/agence` (eller kør `rm -rf ~/.cache/agence`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\agence`

3. Start Agence Desktop på ny.

---

### Løs problemer med serverforbindelse

Agence Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis du har `AGENCE_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `AGENCE_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver Agence Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Agence.

---

### Meddelelser vises ikke

Agence Desktop viser kun systemvarsler når:

- varsler er aktiveret for Agence i dine OS-indstillinger, og
...
1. Afslut Agence Desktop.
2. Find og slet disse filer (de findes i Agence Desktop-appens datamappe):

- `agence.settings.dat` (skrivebordsstandardserver URL)
- `agence.global.dat` og `agence.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Agence:
...
For at finde ud af hvilke modeller du har adgang til, kør `agence models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\agence`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. agence installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\agence`

2. Start agence på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge agence til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
agence vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
