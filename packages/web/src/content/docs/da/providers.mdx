---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Agence.
---

import config from "../../../../config.mjs"
export const console = config.console

Agence bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Agence-konfigurationen.
...
Du kan tilpasse udbyderne gennem `provider`-delen i Agence-konfigurationen.
...
Agence Zen er en liste over modeller leveret af Agence-teamet, som er blevet testet og verificeret til at fungere godt med Agence. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Agence Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg agence og gå til [agence.ai/auth](https://github.com/David2024patton/agence/auth).
...
Det fungerer som alle andre udbydere i Agence og er helt valgfrit at bruge.

---

## Agence Go

Agence Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Agence-teamet, som er testet og verificeret til at fungere godt med Agence.

1. Kør kommandoen `/connect` i TUI, vælg `Agence Go`, og gå til [agence.ai/auth](https://github.com/David2024patton/agence/zen).
...
Sådan bruger du Amazon Bedrock med Agence:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører agence:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Agence understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere agence til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at agence skal fungere korrekt.
   :::
...
gemme tokenet i agence auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Agence bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Agence
...
Sådan bruger du GitHub Copilot-abonnementet med Agence:
...
Sådan bruger du Google Vertex AI med Agence:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem agence, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk at logge hver Agence-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Agence-samtale opført som en separat session.
...
Du kan konfigurere agence til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere agence til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere agence til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Agence. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/agence) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Agence:
...
3. Kopier API-nøglen til brug i Agence.
...
6. **Vigtigt**: Før du bruger skymodeller i Agence, skal du hente modelinformationen lokalt:
...
Agence Zen er en liste over testede og verificerede modeller leveret af Agence-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Agence Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Agence Zen**.

   ```txt
   /connect
   ```

3. Indtast Agence API-nøglen.
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Agence:
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med agence. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
`limit`-felterne lader Agence forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `agence auth list` for at se om legitimationsoplysningerne
