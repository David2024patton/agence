...
Her er alle de innebygde verktøyene som er tilgængelige i Agence.
...
:::note
Dette verktøyet er bare tilgængelig når `AGENCE_EXPERIMENTAL_LSP_TOOL=true` (eller `AGENCE_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger Agence-leverandøren eller når miljøvariabelen `AGENCE_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
