---
title: TUI
description: Utilisation de l’interface utilisateur du terminal Agence.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Agence fournit une interface de terminal interactif ou TUI pour travailler sur vos projets avec un LLM.

L’exécution de Agence démarre le TUI pour le répertoire actuel.
...
Lorsque vous utilisez le Agence TUI, vous pouvez taper `/` suivi d'un nom de commande pour exécuter rapidement des actions. Par exemple:
...
Ajoutez un fournisseur à Agence. Vous permet de sélectionner parmi les fournisseurs disponibles et d’ajouter leurs clés API.
...
- `leader_timeout` - Contrôle la durée pendant laquelle Agence attend après la leader key. Valeur par défaut : `2000`.
...
- `sound_pack` - ID du sound pack à utiliser. Valeur par défaut : `agence.default`.
