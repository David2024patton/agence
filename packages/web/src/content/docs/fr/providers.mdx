---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en Agence.
---

import config from "../../../../config.mjs"
export const console = config.console

Agence utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration Agence.
...
Vous pouvez personnaliser les fournisseurs via la section `provider` de votre Agence
configuration.
...
Agence Zen est une liste de modèles fournis par l'équipe Agence qui ont été
testé et vérifié pour fonctionner correctement avec Agence. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par Agence Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez agence et dirigez-vous vers [agence.ai/auth](https://github.com/David2024patton/agence/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans Agence et son utilisation est totalement facultative.

---

## Agence Go

Agence Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe Agence qui ont été
testé et vérifié pour fonctionner correctement avec Agence.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `Agence Go` et rendez-vous sur [agence.ai/auth](https://github.com/David2024patton/agence/zen).
...
Pour utiliser Amazon Bedrock avec Agence :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de agence :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans Agence n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
Vous pouvez configurer agence pour utiliser des modèles locaux via [Atomic Chat](https://atomic.chat), une application de bureau qui exécute des LLM locaux derrière un serveur API compatible OpenAI (point de terminaison par défaut `http://127.0.0.1:1337/v1`).
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que agence fonctionne correctement.
   :::
...
Vous pouvez également ajouter des modèles via votre configuration agence.
...
pour stocker le jeton dans le stockage d'authentification agence.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
Agence utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller Agence
...
Pour utiliser votre abonnement GitHub Copilot avec agence :
...
Pour utiliser Google Vertex AI avec Agence :
...
Définissez-les lors de l'exécution de agence.
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via agence, vous pouvez toujours le configurer vous-même.
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) pour enregistrer automatiquement chaque conversation Agence en tant que session dans Helicone.
...
Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation Agence répertoriée comme une session distincte.
...
Vous pouvez configurer agence pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)
...
Vous pouvez configurer agence pour utiliser des modèles locaux via LM Studio.
...
Vous pouvez configurer agence pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour Agence. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/agence) pour plus de détails.
:::
...
Pour utiliser Ollama Cloud avec Agence :
...
3. Copiez la clé API à utiliser dans Agence.
...
6. **Important** : Avant d'utiliser des modèles cloud dans Agence, vous devez extraire les informations du modèle localement :
...
Agence Zen est une liste de modèles testés et vérifiés fournie par l'équipe Agence. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>Agence Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **Agence Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé Agence API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration agence.
...
5. Vous pouvez également les personnaliser via votre configuration agence. Voici un exemple de spécification d'un fournisseur
...
Vous pouvez également personnaliser les modèles via votre configuration agence. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec agence. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
Les champs `limit` permettent à Agence de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `agence auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration agence et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration agence.
