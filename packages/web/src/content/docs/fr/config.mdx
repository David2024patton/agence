---
title: Configuration
description: Utilisation de la configuration JSON Agence.
---

Vous pouvez configurer Agence à l'aide d'un fichier de configuration JSON.

---

## Format

Agence prend en charge les formats **JSON** et **JSONC** (JSON avec commentaires).
...
1. **Configuration distante** (à partir de `.well-known/agence`) - paramètres par défaut de l'organisation
2. **Configuration globale** (`~/.config/opencode/opencode.json`) - préférences utilisateur
3. **Configuration personnalisée** (`AGENCE_CONFIG` env var) - remplacements personnalisés
...
6. **Configuration en ligne** (`AGENCE_CONFIG_CONTENT` env var) - remplacements d'exécution
...
:::note
Les répertoires `.opencode` et `~/.config/agence` utilisent des **noms au pluriel** pour les sous-répertoires : `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` et `themes/`. Les noms singuliers (par exemple, `agent/`) sont également pris en charge pour une compatibilité ascendante.
:::
...
Les organisations peuvent fournir une configuration par défaut via le point de terminaison `.well-known/agence`. Ceci est récupéré automatiquement lorsque vous vous authentifiez auprès d’un fournisseur qui le prend en charge.
...
Placez votre configuration globale Agence dans `~/.config/opencode/opencode.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les fournisseurs, les modèles et les autorisations.
...
Lorsque Agence démarre, il recherche un fichier de configuration dans le répertoire actuel ou remonte jusqu'au répertoire Git le plus proche.
...
Spécifiez un chemin de fichier de configuration personnalisé à l'aide de la variable d'environnement `AGENCE_CONFIG`.
...
Spécifiez un répertoire de configuration personnalisé à l'aide de `AGENCE_CONFIG_DIR` variable d'environnement. Ce répertoire sera recherché pour les agents, les commandes, modes et plugins tout comme le répertoire standard `.opencode`, et devrait suivre la même structure.
...
Le fichier de configuration a un schéma défini dans [**`agence.ai/config.json`**](https://github.com/David2024patton/agence/config.json).

La configuration TUI utilise [**`agence.ai/tui.json`**](https://github.com/David2024patton/agence/tui.json).
...
Utilisez `AGENCE_TUI_CONFIG` pour pointer vers un fichier de configuration TUI personnalisé.
...
Vous pouvez configurer les paramètres du serveur pour les commandes `agence serve` et `agence web` via l'option `server`.
...
- `mdns` - Activer la découverte du service mDNS. Cela permet à d'autres appareils du réseau de découvrir votre serveur Agence.
- `mdnsDomain` - Nom de domaine personnalisé pour le service mDNS. La valeur par défaut est `agence.local`. Utile pour exécuter plusieurs instances sur le même réseau.
...
Vous pouvez configurer les fournisseurs et les modèles que vous souhaitez utiliser dans votre configuration Agence via les options `provider`, `model` et `small_model`.
...
L'option `small_model` configure un modèle distinct pour les tâches légères comme la génération de titres. Par défaut, Agence essaie d'utiliser un modèle moins cher s'il est disponible auprès de votre fournisseur, sinon il revient à votre modèle principal.
...
L'agent par défaut doit être un agent principal (et non un sous-agent). Il peut s'agir d'un agent intégré tel que `"build"` ou `"plan"`, ou d'un [agent personnalisé](/docs/agents) que vous avez défini. Si l'agent spécifié n'existe pas ou est un sous-agent, Agence reviendra à `"build"` avec un avertissement.

Ce paramètre s'applique à toutes les interfaces : TUI, CLI (`agence run`), application de bureau et GitHub Action.
...
Agence téléchargera automatiquement toutes les nouvelles mises à jour au démarrage. Vous pouvez désactiver cela avec l'option `autoupdate`.
...
Par défaut, agence **autorise toutes les opérations** sans nécessiter d'approbation explicite. Vous pouvez modifier cela en utilisant l'option `permission`.
...
[Plugins](/docs/plugins) étendent Agence avec des outils, des hooks et des intégrations personnalisés.
...
Ceci est utile lorsque vous souhaitez restreindre Agence à l'utilisation de fournisseurs spécifiques plutôt que de les désactiver un par un.
