---
title: Serveur
description: Interagissez avec le serveur agence via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `agence serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client agence peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `agence.local` |
...
Définissez `AGENCE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `agence`, ou définissez `AGENCE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `agence serve` et à `agence web`.
...
Lorsque vous exécutez `agence`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur agence pour interagir avec agence par programmation.
:::

Cette architecture permet à agence de prendre en charge plusieurs clients et vous permet d'interagir avec agence par programme.

Vous pouvez exécuter `agence serve` pour démarrer un serveur autonome. Si vous avez le
agence TUI en cours d'exécution, `agence serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Agence [IDE](/docs/ide).
...
Le serveur agence expose les API suivantes.
