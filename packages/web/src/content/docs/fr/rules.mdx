---
title: Règles
description: Définissez des instructions personnalisées pour agence.
---

Vous pouvez fournir des instructions personnalisées à agence en créant un fichier `AGENTS.md`. Ceci est similaire aux règles du curseur. Il contient des instructions qui seront incluses dans le contexte du LLM pour personnaliser son comportement pour votre projet spécifique.

---

## Initialiser

Pour créer un nouveau fichier `AGENTS.md`, vous pouvez exécuter la commande `/init` dans agence.

:::tip
Vous devez valider le fichier `AGENTS.md` de votre projet dans Git.
:::

Cela analysera votre projet et tout son contenu pour comprendre de quoi il s'agit et générera un fichier `AGENTS.md` avec celui-ci. Cela aide agence à mieux naviguer dans le projet.
...
agence prend également en charge la lecture du fichier `AGENTS.md` à partir de plusieurs emplacements. Et cela répond à des objectifs différents.
...
Vous pouvez également avoir des règles globales dans un fichier `~/.config/opencode/AGENTS.md`. Ceci est appliqué à toutes les sessions agence.
...
Pour les utilisateurs migrant depuis Claude Code, Agence prend en charge les conventions de fichiers de Claude Code comme solutions de secours :
...
```bash
export AGENCE_DISABLE_CLAUDE_CODE=1        # Disable all .claude support
export AGENCE_DISABLE_CLAUDE_CODE_PROMPT=1 # Disable only ~/.claude/CLAUDE.md
export AGENCE_DISABLE_CLAUDE_CODE_SKILLS=1 # Disable only .claude/skills
```
...
Lorsque agence démarre, il recherche les fichiers de règles dans cet ordre :
...
Bien que agence n'analyse pas automatiquement les références de fichiers dans `AGENTS.md`, vous pouvez obtenir des fonctionnalités similaires de deux manières :
...
Vous pouvez apprendre à agence à lire des fichiers externes en fournissant des instructions explicites dans votre `AGENTS.md`. Voici un exemple pratique :
...
- Assurez-vous que agence charge les fichiers uniquement lorsque cela est nécessaire pour une tâche spécifique.
