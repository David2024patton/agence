...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Agence posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Agence.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `AGENCE_EXPERIMENTAL_LSP_TOOL=true` (lub `AGENCE_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Agence lub gdy zmienna środowiskowa `AGENCE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Agence:
