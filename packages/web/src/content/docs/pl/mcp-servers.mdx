...
Możesz dodać dodatkowe informacje do agence za pomocą _Model Context Protocol_, w skrócie MCP. agence obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [agence Config](https://github.com/David2024patton/agence/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/agence`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
agence automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, agence:
...
Jeśli serwer wymaga uwierzytelnienia, agence poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `agence mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji agence bezpieczne przechowa tokeny w `~/.local/share/opencode/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w agence, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie agence, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć agence z kontem Sentry.
