---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w agence.
---

import config from "../../../../config.mjs"
export const console = config.console

agence używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji agence.
...
Możesz dostosować dostawców za pomocą sekcji `provider` w swoim agence
config.
...
Agence Zen to lista modeli dostarczonych przez zespół agence, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z agence. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Agence Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Agence Zen` i przejdź do [agence.ai/auth](https://github.com/David2024patton/agence/zen).
...
Działa jak każdy inny dostawca w agence i jest całkowicie opcjonalny w użyciu.

---

## Agence Go

Agence Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół agence, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z agence.

1. Uruchom polecenie `/connect` w TUI, wybierz `Agence Go` i przejdź do [agence.ai/auth](https://github.com/David2024patton/agence/zen).
...
Aby używać Amazon Bedrock z agence:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania agence:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w agence nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować agence do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod agence działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji agence.
...
agence używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować agence
...
Aby korzystać z subskrypcji GitHub Copilot z kodem agence:
...
Aby używać Google Vertex AI z agence:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania agence.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą agence, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session), aby automatycznie rejestrować każdą konwersację agence jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja agence będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować agence tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować agence tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować agence tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem agence. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/agence).
:::
...
Aby korzystać z Ollama Cloud z agence:
...
3. Skopiuj klucz API do użycia w agence.
...
6. **Ważne**: Przed użyciem modeli chmurowych w agence musisz pobrać informacje o modelu lokalnie:
...
Agence Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół agence. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Agence Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Agence Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API agence.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji agence.
...
5. Możesz je także dostosować za pomocą konfiguracji agence. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z agence:
...
Możesz także dostosować modele za pomocą konfiguracji agence. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z agence. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
Pola `limit` pozwalają agence zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `agence auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację agence i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji agence.
