---
title: Serwer
description: Interakcja z serwerem agence za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `agence serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient agence.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `agence.local` |
...
Ustaw `AGENCE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `agence` lub ustaw `AGENCE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `agence serve`, jak i `agence web`.
...
Kiedy uruchomisz `agence`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera agence do programowej interakcji z agence.
:::

Ta architektura umożliwia obsługę wielu klientów przez agence i programową interakcję z kodem otwartym.

Możesz uruchomić `agence serve`, aby uruchomić samodzielny serwer. Jeśli masz
agence TUI działa, `agence serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki agence [IDE](/docs/ide).
...
Serwer agence udostępnia następujące interfejsy API.
