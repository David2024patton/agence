---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć agence.
---

Wtyczki umożliwiające wyłączenie agence poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania agence.
...
agence uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z agence do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do agence.
...
:::note
Jeśli korzystasz z aplikacji komputerowej agence, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj agence czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do agence:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod agence. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://github.com/David2024patton/agence/docs/sdk).
