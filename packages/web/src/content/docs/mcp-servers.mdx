...
You can add external tools to Agence using the _Model Context Protocol_, or MCP. Agence supports both local and remote servers.
...
You can define MCP servers in your [Agence Config](https://github.com/David2024patton/agence/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/agence` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Agence automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Agence will:
...
If the server requires authentication, Agence will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `agence mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Agence will store the tokens securely in `~/.local/share/opencode/mcp-auth.json`.
...
Your MCPs are available as tools in Agence, alongside built-in tools. So you can manage them through the Agence config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Agence to your Sentry account.
