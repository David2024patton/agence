---
title: TUI
description: Using the Agence terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Agence provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Agence starts the TUI for the current directory.
...
When using the Agence TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Agence. Allows you to select from available providers and add their API keys.
...
- `leader_timeout` - Controls how long Agence waits after the leader key. Defaults to `2000`.
...
Use `AGENCE_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `agence.default`.
