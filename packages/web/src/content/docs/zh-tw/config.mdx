---
title: 設定
description: 使用 Agence JSON 設定。
---

您可以使用 JSON 設定檔來設定 Agence。

---

## 格式

Agence 支援 **JSON** 和 **JSONC**（帶註解的 JSON）格式。
...
1. **遠端設定**（來自 `.well-known/agence`）- 組織預設值
2. **全域設定**（`~/.config/opencode/opencode.json`）- 使用者偏好
3. **自訂設定**（`AGENCE_CONFIG` 環境變數）- 自訂覆寫
4. **專案設定**（專案中的 `opencode.json`）- 專案特定設定
5. **`.opencode` 目錄** - 代理、指令、外掛程式
6. **內嵌設定**（`AGENCE_CONFIG_CONTENT` 環境變數）- 執行時覆寫

這意味著專案設定可以覆寫全域預設值，全域設定可以覆寫遠端組織預設值。

:::note
`.opencode` 和 `~/.config/agence` 目錄的子目錄使用**複數名稱**：`agents/`、`commands/`、`modes/`、`plugins/`、`skills/`、`tools/` 和 `themes/`。為了向後相容，也支援單數名稱（例如 `agent/`）。
:::
...
組織可以透過 `.well-known/agence` 端點提供預設設定。當您使用支援該功能的供應商進行身分驗證時，會自動擷取此設定。
...
將全域 Agence 設定放在 `~/.config/opencode/opencode.json` 中。使用全域設定來設定使用者層級的偏好，例如主題、供應商或快捷鍵。
...
當 Agence 啟動時，它會在當前目錄中尋找設定檔，或向上遍歷到最近的 Git 目錄。
...
使用 `AGENCE_CONFIG` 環境變數指定自訂設定檔路徑。
...
使用 `AGENCE_CONFIG_DIR` 環境變數指定自訂設定目錄。該目錄會像標準 `.opencode` 目錄一樣被搜尋代理、指令、模式和外掛程式，並且應遵循相同的結構。
...
設定檔具有在 [**`agence.ai/config.json`**](https://github.com/David2024patton/agence/config.json) 中定義的 Schema。
...
使用 `AGENCE_TUI_CONFIG` 指向自訂 TUI 設定檔。
...
您可以透過 `server` 選項為 `agence serve` 和 `agence web` 指令設定伺服器設定。
...
- `mdns` - 啟用 mDNS 服務探索。這允許網路上的其他裝置發現您的 Agence 伺服器。
- `mdnsDomain` - mDNS 服務的自訂網域名稱。預設為 `agence.local`。適用於在同一網路上執行多個實例的情境。
...
您可以透過 `provider`、`model` 和 `small_model` 選項在 Agence 設定中設定要使用的供應商和模型。
...
`small_model` 選項為標題生成等輕量級任務設定單獨的模型。預設情況下，如果您的供應商有更便宜的模型可用，Agence 會嘗試使用該模型，否則會回退到您的主模型。
...
預設代理必須是主代理（不能是子代理）。可以是內建代理（如 `"build"` 或 `"plan"`），也可以是您定義的[自訂代理](/docs/agents)。如果指定的代理不存在或是子代理，Agence 將回退到 `"build"` 並發出警告。

此設定適用於所有介面：TUI、CLI（`agence run`）、桌面應用程式和 GitHub Action。
...
Agence 啟動時會自動下載新版本。您可以使用 `autoupdate` 選項停用此功能。
...
預設情況下，Agence **允許所有操作**，無需明確批准。您可以使用 `permission` 選項變更此行為。
...
[外掛程式](/docs/plugins)透過自訂工具、掛鉤和整合來擴展 Agence。
...
當您希望限制 Agence 僅使用特定供應商，而不是逐一停用其他供應商時，此選項非常有用。
