---
title: Konfigurasjon
description: Ved å bruke agence JSON-konfigurasjonen.
---

Du kan konfigurere Agence ved å bruke en JSON konfigurasjonsfil.

---

## Format

Agence støtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
For eksempel, hvis din globale konfigurasjon setter `theme: "agence"` og `autoupdate: true`, og prosjektkonfigurasjonen setter `model: "anthropic/claude-sonnet-4-5"`, vil den endelige konfigurasjonen inkludere alle tre innstillingene.
...
1. **Ekstern konfigurasjon** (fra `.well-known/agence`) - organisasjonsstandarder
2. **Global konfigurasjon** (`~/.config/opencode/opencode.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`AGENCE_CONFIG` env var) - egendefinerte overstyringer
...
6. **Inline config** (`AGENCE_CONFIG_CONTENT` env var) - kjøretidsoverstyringer
...
:::note
`.opencode` og `~/.config/agence` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Organisasjoner kan tilby standardkonfigurasjon via `.well-known/agence`-endepunktet. Dette hentes automatisk når du autentiserer med en leverandør som støtter det.
...
Plasser din globale Agence-konfigurasjon i `~/.config/opencode/opencode.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Når Agence starter opp, ser den etter en konfigurasjonsfil i gjeldende katalog eller går opp til nærmeste Git-katalog.
...
Spesifiser en tilpasset konfigurasjonsfilbane ved å bruke miljøvariabelen `AGENCE_CONFIG`.
...
Spesifiser en tilpasset konfigurasjonskatalog ved å bruke `AGENCE_CONFIG_DIR`
...
Konfigurasjonsfilen har et skjema som er definert i [**`agence.ai/config.json`**](https://github.com/David2024patton/agence/config.json).
...
Du kan konfigurere serverinnstillinger for kommandoene `agence serve` og `agence web` gjennom alternativet `server`.
...
- `mdns` - Aktiver mDNS-tjenesteoppdagelse. Dette lar andre enheter på nettverket oppdage Agence-serveren din.
- `mdnsDomain` - Egendefinert domenenavn for mDNS-tjeneste. Standard er `agence.local`. Nyttig for å kjøre flere forekomster på samme nettverk.
...
Du kan konfigurere leverandørene og modellene du vil bruke i Agence-konfigurasjonen gjennom alternativene `provider`, `model` og `small_model`.
...
Alternativet `small_model` konfigurerer en egen modell for lette oppgaver som tittelgenerering. Som standard prøver Agence å bruke en rimeligere modell hvis en er tilgjengelig fra leverandøren din, ellers faller den tilbake til hovedmodellen din.
...
Standardagenten må være en primæragent (ikke en underagent). Dette kan være en innebygd agent som `"build"` eller `"plan"`, eller en [egendefinert agent](/docs/agents) du har definert. Hvis den angitte agenten ikke eksisterer eller er en subagent, vil Agence falle tilbake til `"build"` med en advarsel.

Denne innstillingen gjelder for alle grensesnitt: TUI, CLI (`agence run`), skrivebordsapp og GitHub Action.
...
Agence vil automatisk laste ned eventuelle nye oppdateringer når den starter opp. Du kan deaktivere dette med alternativet `autoupdate`.
...
Som standard **tillater Agence alle operasjoner** uten å kreve eksplisitt godkjenning. Du kan endre dette ved å bruke alternativet `permission`.
...
[Plugins](/docs/plugins) utvider Agence med tilpassede verktøy, kroker og integrasjoner.
...
Dette er nyttig når du vil begrense Agence til kun å bruke spesifikke leverandører i stedet for å deaktivere dem én etter én.
