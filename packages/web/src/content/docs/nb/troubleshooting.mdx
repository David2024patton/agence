...
For å feilsøke problemer med Agence, start med å sjekke loggene og lokale data den lagrer på disken.
...
- **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.local\share\agence\log`

Loggfiler navngis med tidsstempler (f.eks. `2025-01-09T123456.log`) og de siste 10 loggfilene beholdes.

Du kan angi loggnivået med kommandolinjealternativet `--log-level` for å få mer detaljert feilsøkingsinformasjon. For eksempel `agence --log-level DEBUG`.

---

## Lagring

Agence lagrer øktdata og andre applikasjonsdata på disken på:

- **macOS/Linux**: `~/.local/share/opencode/`
- **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.local\share\agence`
...
Agence Desktop kjører en lokal Agence-server (`opencode-cli` sidevognen) i bakgrunnen. De fleste problemene er forårsaket av en plugin som fungerer dårlig, en ødelagt cache eller en dårlig serverinnstilling.

### Raske sjekker

- Avslutt og start appen på nytt.
- Hvis appen viser en feilskjerm, klikker du på **Start på nytt** og kopierer feildetaljene.
- Bare macOS: `Agence`-meny -> **Last nettvisning på nytt** (hjelper hvis UI er tom/frosset).
...
- **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.config\agence\opencode.jsonc`
...
Agence kan også laste lokale plugins fra disken. Flytt disse midlertidig ut av veien (eller gi nytt navn til mappen) og start skrivebordsappen på nytt:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/opencode/plugins/`
  - **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.config\agence\plugins`
...
Hvis deaktivering av plugins ikke hjelper (eller en plugin-installasjon sitter fast), tøm hurtigbufferen slik at Agence kan gjenoppbygge den.

1. Avslutt Agence Desktop helt.
2. Slett hurtigbufferkatalogen:

- **macOS**: Finder -> `Cmd+Shift+G` -> lim inn `~/.cache/agence`
- **Linux**: slett `~/.cache/agence` (eller kjør `rm -rf ~/.cache/agence`)
- **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.cache\agence`

3. Start Agence Desktop på nytt.

---

### Løs problemer med servertilkobling

Agence Desktop kan enten starte sin egen lokale server (standard) eller koble til en server URL du har konfigurert.
...
Hvis du har `AGENCE_PORT` satt i miljøet ditt, vil skrivebordsappen prøve å bruke den porten for den lokale serveren.

- Deaktiver `AGENCE_PORT` (eller velg en ledig port) og start på nytt.
...
På Windows krever Agence Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åpnes i et tomt vindu eller ikke starter, installer/oppdater WebView2 og prøv igjen.

---

### Windows: Generelle ytelsesproblemer

Hvis du opplever treg ytelse, problemer med filtilgang eller terminalproblemer på Windows, kan du prøve å bruke [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL gir et Linux-miljø som fungerer mer sømløst med funksjonene til Agence.

---

### Varsler vises ikke

Agence Desktop viser bare systemvarsler når:

- varsler er aktivert for Agence i OS-innstillingene dine, og
...
1. Avslutt Agence Desktop.
2. Finn og slett disse filene (de finnes i Agence Desktop-appens datakatalog):

- `agence.settings.dat` (stasjonær standardserver URL)
- `agence.global.dat` og `agence.workspace.*.dat` (UI tilstand som nylige servere/prosjekter)
...
Hvis du støter på API-anropsfeil, kan dette skyldes utdaterte provider-pakker. Agence installerer dynamisk provider-pakker (OpenAI, Anthropic, Google, etc.) etter behov og cacher dem lokalt.
...
1. Tøm leverandørens pakkebuffer:
   ...
   På Windows, trykk `WIN+R` og slett: `%USERPROFILE%\.cache\agence`

2. Start Agence på nytt for å installere de nyeste provider-pakkene på nytt

Dette vil tvinge Agence til å laste ned de nyeste versjonene av provider-pakkene, som ofte løser kompatibilitetsproblemer med modellparametere og API-endringer.
...
Agence vil oppdage om du bruker Wayland og foretrekker `wl-clipboard`, ellers vil den prøve å finne utklippstavle-verktøy i rekkefølgen: `xclip` og `xsel`.
