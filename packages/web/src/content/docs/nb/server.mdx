---
title: Tjener
description: Samhandle med Agence-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `agence serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Agence-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `agence.local` |
...
Sett `AGENCE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `agence` som standard, eller sett `AGENCE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `agence serve` og `agence web`.
...
Når du kjører `agence` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Agence-serveren til å samhandle med Agence programmatisk.
:::

Denne arkitekturen lar Agence støtte flere klienter og lar deg samhandle med Agence programmatisk.

Du kan kjøre `agence serve` for å starte en frittstående server. Hvis du har
Agence TUI kjører, vil `agence serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Agence [IDE](/docs/ide) plugins.
