...
Agence gir et interaktivt terminalgrensesnitt (TUI) for å jobbe med prosjektene dine med en LLM.

Å kjøre Agence starter TUI for gjeldende katalog.
...
Når du bruker Agence TUI, kan du skrive `/` etterfulgt av et kommandonavn for raskt å utføre handlinger. For eksempel:
...
Legg til en leverandør til Agence. Lar deg velge fra tilgjengelige leverandører og legge til deres API-nøkler.
...
Avslutt Agence. _Aliaser_: `/quit`, `/q`
...
- `leader_timeout` - Styrer hvor lenge Agence venter etter leader key. Standard er `2000`.
...
- `sound_pack` - Sound pack-ID som skal brukes. Standard er `agence.default`.
