---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Agence.
---

import config from "../../../../config.mjs"
export const console = config.console

Agence bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Agence-konfigurasjonen.
...
Du kan tilpasse leverandørene gjennom `provider`-delen i Agence
konfig.
...
Agence Zen er en liste over modeller levert av Agence-teamet som har vært
testet og verifisert for å fungere godt med Agence. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Agence Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Agence Zen` og gå til [agence.ai/auth](https://github.com/David2024patton/agence/zen).
...
Det fungerer som alle andre leverandører i Agence og er helt valgfritt å bruke.

---

## Agence Go

Agence Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Agence-teamet som har vært
testet og verifisert for å fungere godt med Agence.

1. Kjør kommandoen `/connect` i TUI, velg `Agence Go`, og gå til [agence.ai/auth](https://github.com/David2024patton/agence/zen).
...
Slik bruker du Amazon Bedrock med Agence:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Agence:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i agence støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere agence til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Agence skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Agence-konfigurasjonen.
...
for å lagre token i Agence auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Agence bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Agence
...
Slik bruker du GitHub Copilot-abonnementet med Agence:
...
Slik bruker du Google Vertex AI med Agence:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører Agence.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Agence, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk å logge hver Agence-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Agence-samtale oppført som en separat økt.
...
Du kan konfigurere Agence for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Agence for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Agence for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Agence. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/agence) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Agence:
...
3. Kopier API-nøkkelen for bruk i Agence.
...
6. **Viktig**: Før du bruker skymodeller i Agence, må du hente modellinformasjonen lokalt:
...
Agence Zen er en liste over testede og verifiserte modeller levert av Agence-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Agence Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Agence Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Agence API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Agence-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Agence-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Agence:
...
Du kan også tilpasse modeller gjennom Agence-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Agence. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
`limit`-feltene lar Agence forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `agence auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Agence-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Agence-konfigurasjonen.
