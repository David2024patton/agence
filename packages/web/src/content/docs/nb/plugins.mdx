...
description: Skriv dine egne programtillegg for å utvide Agence.
---

Plugins lar deg utvide Agence ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Agence.
...
Agence kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Agence SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Agence.
...
:::note
Hvis du bruker Agence-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Agence fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Agence:
...
`tool`-hjelperen lager et tilpasset verktøy som Agence kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Agence sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://github.com/David2024patton/agence/docs/sdk) for detaljer.
