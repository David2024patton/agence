...
Du kan legge til eksterne verktøy i Agence ved å bruke _Model Context Protocol_ eller MCP. Agence støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Agence Config](https://github.com/David2024patton/agence/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/agence`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Agence håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Agence:
...
Hvis serveren krever autentisering, vil Agence be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `agence mcp auth <server-name>`.
...
```bash
agence mcp auth my-oauth-server
```
...
```bash
agence mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Agence lagre tokens sikkert i `~/.local/share/opencode/mcp-auth.json`.
...
agence mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Agence, sammen med innebygde verktøy. Så du kan administrere dem gjennom Agence-konfigurasjonen som alle andre verktøy.
...
```bash
agence mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Agence til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Agence-samtale oppført som en separat økt.
