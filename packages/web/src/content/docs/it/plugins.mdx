---
title: Plugin
description: Scrivi plugin per estendere Agence.
---

I plugin ti permettono di estendere Agence agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Agence.
...
Agence esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di agence per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere agence.
...
:::note
Se usi l'app desktop di Agence, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a agence di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a agence:
...
L'helper `tool` crea uno strumento personalizzato che agence può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in agence insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://github.com/David2024patton/agence/docs/sdk) per i dettagli.
