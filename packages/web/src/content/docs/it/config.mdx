...
description: Usa la configurazione JSON di Agence.
---

Puoi configurare Agence usando un file di configurazione JSON.

---

## Formato

Agence supporta sia **JSON** sia **JSONC** (JSON con commenti).
...
1. **Config remota** (da `.well-known/agence`) - default dell'organizzazione
2. **Config globale** (`~/.config/opencode/opencode.json`) - preferenze utente
3. **Config personalizzata** (variabile d'ambiente `AGENCE_CONFIG`) - sovrascritture personalizzate
...
6. **Config inline** (variabile d'ambiente `AGENCE_CONFIG_CONTENT`) - sovrascritture a runtime
...
:::note
Le directory `.opencode` e `~/.config/agence` usano **nomi al plurale** per le sottodirectory: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` e `themes/`. I nomi al singolare (ad es. `agent/`) sono supportati per retrocompatibilita.
:::
...
Le organizzazioni possono fornire una configurazione predefinita tramite l'endpoint `.well-known/agence`. Viene recuperata automaticamente quando ti autentichi con un provider che lo supporta.
...
Metti la configurazione globale di Agence in `~/.config/opencode/opencode.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.
...
Quando Agence si avvia, cerca un file di configurazione nella directory corrente oppure risale fino alla directory Git piu vicina.
...
Specifica un percorso personalizzato per il file di configurazione usando la variabile d'ambiente `AGENCE_CONFIG`.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `AGENCE_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.opencode` e dovrebbe seguire la stessa struttura.
...
Il file di configurazione ha uno schema definito in [**`agence.ai/config.json`**](https://github.com/David2024patton/agence/config.json).

La configurazione TUI usa [**`agence.ai/tui.json`**](https://github.com/David2024patton/agence/tui.json).
...
Usa `AGENCE_TUI_CONFIG` per puntare a un file di configurazione TUI personalizzato.
...
Puoi configurare le impostazioni del server per i comandi `agence serve` e `agence web` tramite l'opzione `server`.
...
- `mdns` - Abilita la scoperta del servizio mDNS. Questo permette ad altri dispositivi sulla rete di scoprire il tuo server Agence.
- `mdnsDomain` - Nome di dominio personalizzato per il servizio mDNS. Predefinito: `agence.local`. Utile per eseguire piu istanze sulla stessa rete.
...
Puoi configurare provider e modelli da usare in Agence tramite le opzioni `provider`, `model` e `small_model`.
...
L'opzione `small_model` configura un modello separato per task leggeri come la generazione dei titoli. Per impostazione predefinita, Agence prova a usare un modello più economico se disponibile dal tuo provider, altrimenti fa fallback sul tuo modello principale.
...
L'agente predefinito deve essere un agente primary (non un subagent). Puo essere un agente integrato come `"build"` o `"plan"`, oppure un [agente personalizzato](/docs/agents) che hai definito. Se l'agente specificato non esiste o e un subagent, Agence fara fallback su `"build"` mostrando un avviso.

Questa impostazione si applica a tutte le interfacce: TUI, CLI (`agence run`), app desktop e GitHub Action.
...
Agence scarichera automaticamente eventuali aggiornamenti quando si avvia. Puoi disabilitare questa funzione con l'opzione `autoupdate`.
...
Per impostazione predefinita, agence **consente tutte le operazioni** senza richiedere un'approvazione esplicita. Puoi cambiarlo usando l'opzione `permission`.
...
[I plugin](/docs/plugins) estendono Agence con strumenti personalizzati, hook e integrazioni.
...
E utile quando vuoi limitare Agence a usare solo alcuni provider invece di disabilitarli uno per uno.
