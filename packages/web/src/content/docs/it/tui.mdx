---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di Agence.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Agence fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire Agence avvia la TUI per la directory corrente.
...
Quando usi la TUI di Agence, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a Agence. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
- `leader_timeout` - Controlla per quanto tempo Agence attende dopo il leader key. Default: `2000`.
...
- `sound_pack` - ID del sound pack da usare. Default: `agence.default`.
