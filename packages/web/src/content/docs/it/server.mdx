---
title: Server
description: Interagisci con il server agence via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `agence serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client agence.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `agence.local` |
...
Imposta `AGENCE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `agence`, oppure imposta `AGENCE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `agence serve` sia per `agence web`.
...
Quando esegui `agence` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server agence per interagire con agence in modo programmatico.
:::

Questa architettura permette a agence di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `agence serve` per avviare un server standalone. Se la TUI di agence e gia in esecuzione, `agence serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Agence.
...
Il server agence espone le seguenti API.
