...
Da biste otklonili probleme s Agence, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\agence\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `agence --log-level DEBUG`.

---

## Pohrana

Agence pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/opencode/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\agence`
...
Agence Desktop pokreće lokalni Agence server (`opencode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Agence` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\agence\opencode.jsonc`
...
Agence također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/opencode/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\agence\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Agence mogao ponovo izgraditi.

1. Potpuno zatvorite Agence Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/agence`
- **Linux**: obrišite `~/.cache/agence` (ili pokrenite `rm -rf ~/.cache/agence`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\agence`

3. Ponovo pokrenite Agence Desktop.

---

### Rješavanje problema sa vezom na serveru

Agence Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako ste postavili `AGENCE_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `AGENCE_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Agence Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Agence karakteristikama.

---

### Obavještenja se ne prikazuju

Agence Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Agence u postavkama vašeg OS-a, i
...
1. Zatvorite Agence Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Agence Desktop aplikacije):

- `agence.settings.dat` (URL zadanog servera za desktop)
- `agence.global.dat` i `agence.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `agence models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\agence`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Agence dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\agence`

2. Ponovo pokrenite Agence da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Agence da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Agence će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
