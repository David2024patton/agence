---
title: Server
description: Komunicirajte s agence serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `agence serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji agence klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `agence.local` |
...
Postavite `AGENCE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `agence`, ili postavite `AGENCE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `agence serve` i za `agence web`.
...
Kada pokrenete `agence`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite agence server za programsku interakciju sa opencode-om.
:::

Ova arhitektura omogucava agence podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `agence serve` da startate standalone server. Ako je agence TUI vec pokrenut, `agence serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Agence [IDE](/docs/ide) pluginovi.
...
agence server izlaže sljedece API-je.
