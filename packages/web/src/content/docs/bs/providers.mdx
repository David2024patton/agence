---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Agence.
---

import config from "../../../../config.mjs"
export const console = config.console

Agence koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Agence konfiguraciji.
...
Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Agence
config.
...
Agence Zen je lista modela koje je obezbedio Agence tim koji su bili
testirano i potvrđeno da dobro radi sa Agence. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Agence Zen.
:::
...
Radi kao i svaki drugi provajder u Agence i potpuno je opcionalan za korištenje.

---

## Agence Go

Agence Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Agence tim i koji su testirani i verificirani da dobro rade s Agence-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Agence Go` i idite na [agence.ai/auth](https://github.com/David2024patton/agence/zen).
...
Da biste koristili Amazon Bedrock s Agence:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete agence:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Agence.
:::
...
Možete konfigurirati agence za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi agence ispravno radio.
   :::
...
Također možete dodati modele kroz svoju agence konfiguraciju.
...
da pohrani token u agence auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Agence koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Agence
...
Da biste koristili svoju GitHub Copilot pretplatu s agence:
...
Za korištenje Google Vertex AI s Agence:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće agence.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) da automatski prijavite svaki Agence razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Agence razgovor naveden kao zasebna sesija.
...
Možete konfigurirati agence za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati agence za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati agence za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Agence. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/agence) za detalje.
:::
...
Da biste koristili Ollama Cloud s Agence:
...
3. Kopirajte API ključ za korištenje u Agence.
...
6. **Važno**: Prije upotrebe modela oblaka u Agence, morate lokalno povući informacije o modelu:
...
Agence Zen je lista testiranih i verifikovanih modela koju je obezbedio Agence tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Agence Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Agence Zen**.

```txt
   /connect
```

3. Unesite svoj Agence API ključ.
...
Također možete dodati dodatne modele putem vaše agence konfiguracije.
...
5. Također ih možete prilagoditi putem vaše agence konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju agence konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
Polja `limit` omogućavaju Agence da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `agence auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Agence konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj agence konfiguraciji.
