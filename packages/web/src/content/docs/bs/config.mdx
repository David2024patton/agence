---
title: Konfiguracija
description: Korištenje Agence JSON konfiguracije.
---

Možete konfigurirati Agence koristeći JSON konfiguracijski fajl.

---

## Format

Agence podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/agence`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/opencode/opencode.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`AGENCE_CONFIG` env var) - prilagođena preinačenja
...
6. **Inline konfiguracija** (`AGENCE_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.opencode` i `~/.config/agence` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/agence` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Agence konfiguraciju u `~/.config/opencode/opencode.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.
...
Kada se Agence pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `AGENCE_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `AGENCE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.opencode` direktorij, i trebali bi pratiti istu strukturu.
...
Konfiguracijski fajl ima šemu koja je definirana u [**`agence.ai/config.json`**](https://github.com/David2024patton/agence/config.json).

TUI konfiguracija koristi [**`agence.ai/tui.json`**](https://github.com/David2024patton/agence/tui.json).
...
Koristite `AGENCE_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.
...
Možete konfigurirati postavke servera za naredbe `agence serve` i `agence web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Agence server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `agence.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Agence konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Agence pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Agence će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`agence run`), desktop aplikaciju i GitHub Action.
...
Agence će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Agence **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Agence sa prilagođenim alatima, kukicama i integracijama.
...
Ovo je korisno kada želite da ograničite Agence da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
