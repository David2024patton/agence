...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Agence dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Agence.
...
:::note
Ovaj alat je dostupan samo kada je `AGENCE_EXPERIMENTAL_LSP_TOOL=true` (ili `AGENCE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Agence provajdera ili kada je varijabla `AGENCE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Agence:
