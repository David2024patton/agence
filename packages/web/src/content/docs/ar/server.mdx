---
title: الخادم
description: تفاعل مع خادم agence عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `agence serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل agence استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `agence.local` |
...
عيّن `AGENCE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `agence`، أو عيّن `AGENCE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `agence serve` و `agence web`.
...
عند تشغيل `agence` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم agence للتفاعل مع agence برمجيا.
:::

تتيح هذه البنية لـ agence دعم عدة عملاء وتمكّنك من التفاعل مع agence برمجيا.

يمكنك تشغيل `agence serve` لبدء خادم مستقل. إذا كانت واجهة agence في terminal (TUI)
قيد التشغيل، فسيبدأ `agence serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Agence لـ [IDE](/docs/ide).
...
يعرض خادم agence واجهات API التالية.
