...
يستخدم Agence إعداد `permission` لتحديد ما إذا كان إجراءٌ ما سيُنفّذ تلقائيًا، أو سيطلب موافقتك، أو سيتم حظره.
...
استخدم `external_directory` للسماح باستدعاءات الأدوات التي تلمس مسارات خارج دليل العمل الذي بدأ منه Agence. ينطبق ذلك على أي أداة تأخذ مسارًا كمدخل (مثل `read` و`edit` و`glob` و`grep` والعديد من أوامر `bash`).
...
تُعرَّف أذونات Agence بأسماء الأدوات، بالإضافة إلى بعض حواجز الأمان:
...
- `websearch` — بحث الويب (يطابق الاستعلام)
...
إذا لم تحدد شيئًا، يبدأ Agence بقيم افتراضية متساهلة:
...
عندما يطلب Agence الموافقة، تعرض الواجهة ثلاث نتائج ممكنة:

- `once` — الموافقة على هذا الطلب فقط
- `always` — الموافقة على الطلبات المستقبلية المطابقة للأنماط المقترحة (لباقي جلسة Agence الحالية)
- `reject` — رفض الطلب
