---
title: المزوّدون
description: استخدام أي موفّر LLM في Agence.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Agence ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Agence.
...
يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Agence.
...
Agence Zen هي قائمة نماذج يوفّرها فريق Agence وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Agence. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Agence Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر agence، ثم انتقل إلى [agence.ai/auth](https://github.com/David2024patton/agence/auth).
...
يعمل مثل أي مزوّد آخر في Agence واستخدامه اختياري بالكامل.

---

## Agence Go

Agence Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Agence، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Agence.

1. شغّل الأمر `/connect` في TUI، واختر `Agence Go`، ثم انتقل إلى [agence.ai/auth](https://github.com/David2024patton/agence/zen).
...
لاستخدام Amazon Bedrock مع Agence:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل agence:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Agence غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين agence لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل agence بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات agence.
...
تخزين الرمز في مساحة تخزين مصادقة agence.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Agence نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Agence
...
لاستخدام اشتراك GitHub Copilot مع agence:
...
لاستخدام Google Vertex AI مع Agence:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر agence، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) لتسجيل كل محادثة Agence تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Agence مدرجة كجلسة مستقلة.
...
يمكنك ضبط agence لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط agence لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط agence لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Agence. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/agence) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Agence:
...
3. انسخ مفتاح API لاستخدامه في Agence.
...
6. **مهم**: قبل استخدام نماذج السحابة في Agence، يجب سحب معلومات النموذج محليا:
...
Agence Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Agence. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Agence Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Agence Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Agence.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات agence.
...
5. يمكنك أيضا تخصيصها عبر إعدادات agence. إليك مثالا على تحديد مزوّد
...
يمكنك أيضا تخصيص النماذج عبر إعدادات agence. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع agence. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
تسمح حقول `limit` لـ Agence بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `agence auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات agence وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات agence.
