...
Der Befehl `agence serve` startet einen headless HTTP-Server.
...
Setze `AGENCE_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `agence`, kann aber mit `AGENCE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `agence serve` und `agence web`.
...
Wenn du `agence` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den opencode-Server, um agence programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `agence serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `agence serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Agence-[IDE](/docs/ide)-Plugins.
