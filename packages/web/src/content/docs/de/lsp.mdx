---
title: LSP Server
description: Agence lässt sich in Ihren LSP-Server integrieren.
---

Agence lässt sich in Ihr Language Server Protocol (LSP) integrieren, um die Interaktion von LLM mit Ihrer Codebasis zu unterstützen. Es verwendet Diagnosefunktionen, um dem LLM Feedback zu geben.

---

## Integriert

Agence verfügt über mehrere integrierte LSP-Server für gängige Sprachen:
...
:::note
Sie können LSP-Server-Downloads automatisch deaktivieren, indem Sie die Umgebungsvariable `AGENCE_DISABLE_LSP_DOWNLOAD` auf `true` setzen.
:::
...
Wenn Agence eine Datei öffnet, geschieht Folgendes:
...
Sie können LSP-Server über den Abschnitt `lsp` in Ihrer Agence-Konfiguration anpassen.
