...
Wenn Agence Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\agence\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `agence --log-level DEBUG`.

---

## Speicher

agence speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/opencode/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\agence`
...
Agence Desktop startet im Hintergrund einen lokalen Agence-Server (`opencode-cli`-Sidecar).
...
- Nur macOS: `Agence`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\agence\opencode.jsonc`
...
Agence kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/opencode/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\agence\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Agence ihn neu aufbauen kann.

1. Beende Agence Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/agence`
- **Linux**: loesche `~/.cache/agence` (oder fuehre aus: `rm -rf ~/.cache/agence`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\agence`

3. Starte Agence Desktop neu.

---

### Server-Verbindungsprobleme beheben

Agence Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn du `AGENCE_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `AGENCE_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt Agence Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Agence-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Agence Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Agence in den OS-Einstellungen aktiviert sind, und
...
1. Beende Agence Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Agence Desktop):

- `agence.settings.dat` (desktop default server URL)
- `agence.global.dat` und `agence.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Agence hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `agence models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\agence`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. agence installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\agence`

2. Starte agence neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt agence, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
agence erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
