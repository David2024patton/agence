---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in Agence.
---

import config from "../../../../config.mjs"
export const console = config.console

Agence verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer Agence-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem Agence anpassen
config.
...
Agence Zen ist eine Liste von Modellen, die vom Agence-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit Agence funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit Agence Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie Agence aus und gehen Sie zu [agence.ai/auth](https://github.com/David2024patton/agence/auth).
...
Es funktioniert wie jeder andere Anbieter in Agence und ist völlig optional.

---

## Agence Go

Agence Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom Agence-Team getestet und verifiziert wurden, dass sie gut mit Agence funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `Agence Go` und gehen Sie zu [agence.ai/auth](https://github.com/David2024patton/agence/zen).
...
So verwenden Sie Amazon Bedrock mit Agence:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie Agence ausführen:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in Agence wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
Sie können agence so konfigurieren, dass es lokale Modelle über [Atomic Chat](https://atomic.chat) verwendet — eine Desktop-Anwendung, die lokale LLMs hinter einem OpenAI-kompatiblen API-Server bereitstellt (Standard-Endpunkt `http://127.0.0.1:1337/v1`).
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit Agence ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre Agence-Konfiguration hinzufügen.
...
um das Token im Agence-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
Agence verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von Agence
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit Agence:
...
So verwenden Sie Google Vertex AI mit Agence:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie Agence ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über Agence konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session)-Plugin, um jede Agence-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede Agence-Konversation als separate Sitzung aufgeführt.
...
Sie können Agence für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können Agence über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können Agence über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für Agence konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/agence).
:::
...
So verwenden Sie Ollama Cloud mit Agence:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in Agence.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in Agence verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
Agence Zen ist eine Liste getesteter und verifizierter Modelle, die vom Agence-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>Agence Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **Agence Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren Agence API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre Agence-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre Agence-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit Agence:
...
Sie können Modelle auch über Ihre Agence-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit Agence verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
Mithilfe der `limit`-Felder kann Agence erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `agence auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die Agence-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer Agence-Konfiguration übereinstimmt.
