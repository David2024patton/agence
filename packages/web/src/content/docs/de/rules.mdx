---
title: Regeln
description: Lege eigene Anweisungen fuer agence fest.
---

Du kannst agence ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in agence den Befehl `/init` aus.
...
So kann agence sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht agence sie zu erweitern.
...
agence kann `AGENTS.md` aus mehreren Orten lesen.
...
Wenn du von Claude Code kommst, unterstuetzt Agence dessen Dateikonventionen als Fallback:
...
Beim Start sucht agence in dieser Reihenfolge nach Regeldateien:
...
agence parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
...
Du kannst agence explizit anweisen, externe Dateien zu lesen.
