---
title: Config
description: Using the Agence JSON config.
---

You can configure Agence using a JSON config file.

---

## Format

Agence supports both **JSON** and **JSONC** (JSON with Comments) formats.
...
1. **Remote config** (from `.well-known/agence`) - organizational defaults
2. **Global config** (`~/.config/opencode/opencode.json`) - user preferences
3. **Custom config** (`AGENCE_CONFIG` env var) - custom overrides
4. **Project config** (`opencode.json` in project) - project-specific settings
5. **`.opencode` directories** - agents, commands, plugins
6. **Inline config** (`AGENCE_CONFIG_CONTENT` env var) - runtime overrides
...
:::note
The `.opencode` and `~/.config/agence` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/agence` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
```json title="Remote config from .well-known/agence"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Place your global Agence config in `~/.config/opencode/opencode.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.
...
When Agence starts up, it looks for a config file in the current directory or traverse up to the nearest Git directory.
...
Specify a custom config file path using the `AGENCE_CONFIG` environment variable.

```bash
export AGENCE_CONFIG=/path/to/my/custom-config.json
agence run "Hello world"
```
...
Specify a custom config directory using the `AGENCE_CONFIG_DIR`
...
```bash
export AGENCE_CONFIG_DIR=/path/to/my/config-directory
agence run "Hello world"
```
...
| Linux    | `/etc/opencode/`                         |
| Windows  | `%ProgramData%\agence`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, Agence reads managed preferences from the `ai.opencode.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

Agence checks these paths:
...
Use the `ai.opencode.managed` PayloadType. The Agence config keys go directly in the payload dict:
...
The server/runtime config schema is defined in [**`agence.ai/config.json`**](https://github.com/David2024patton/agence/config.json).

TUI config uses [**`agence.ai/tui.json`**](https://github.com/David2024patton/agence/tui.json).
...
Use `AGENCE_TUI_CONFIG` to point to a custom TUI config file.
...
You can configure server settings for the `agence serve` and `agence web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your Agence server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `agence.local`. Useful for running multiple instances on the same network.
...
If not specified, Agence will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your Agence config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, Agence tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Agence normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, Agence omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, Agence will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`agence run`), desktop app, and GitHub Action.
...
Agence uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
Agence will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, agence **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend Agence with custom tools, hooks, and integrations.
...
This is useful when you want to restrict Agence to only use specific providers rather than disabling them one by one.
