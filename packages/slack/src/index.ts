...
import { createOpencode, type ToolPart } from "@agence-ai/sdk"
...
console.log("🚀 Starting agence server...")
const agence = await createOpencode({
  port: 0,
})
...
void (async () => {
  const events = await agence.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new agence session...")
    const { client, server } = agence
    ...
    console.log("✅ Created agence session:", createResult.data.id)
  ...
  console.log("📝 Sending to agence:", message.text)
