    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.AGENCE_CHANNEL === "prod" &&
...
const deepLinkEvent = "agence:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__AGENCE__ ??= {}
  const pending = window.__AGENCE__.deepLinks ?? []
  window.__AGENCE__.deepLinks = [...pending, ...urls]
...
render(() => {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("agence.global.dat").getItem("language")
