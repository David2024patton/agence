...
import agence from "./theme/agence.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@agence-ai/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "agence")
        draft.active = typeof active === "string" ? active : "agence"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.agence, store.mode)
    })
