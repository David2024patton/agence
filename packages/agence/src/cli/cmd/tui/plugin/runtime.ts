...
import { Flock } from "@agence-ai/core/util/flock"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".agence"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".agence", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".agence", "tui.json"),
  }
}
...
async function load(input: { api: Api; config: TuiConfig.Resolved; dispose?: () => void; disposeTimeoutMs?: number }) {
  ...
  try {
    ...
    const records = Flag.AGENCE_PURE ? [] : (config.plugin_origins ?? [])
    if (Flag.AGENCE_PURE && config.plugin_origins?.length) {
