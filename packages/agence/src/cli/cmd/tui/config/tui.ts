...
import type { DeepMutable } from "@agence-ai/core/schema"
import type { TuiAttentionSoundName } from "@agence-ai/plugin/tui"
...
export class Service extends Context.Service<Service, Interface>()("@agence/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  // Every config dir we may read from: global config dir, any `.agence`
  // folders between cwd and home, and AGENCE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.AGENCE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit AGENCE_TUI_CONFIG override, if set.
  if (Flag.AGENCE_TUI_CONFIG) {
    const configFile = Flag.AGENCE_TUI_CONFIG
  ...
  // 4. `.agence` directories (and AGENCE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".agence") || dir === Flag.AGENCE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".agence") && dir !== Flag.AGENCE_CONFIG_DIR) continue
  ...
  const result: Resolved = {
    ...
    attention: {
      ...
      sound_pack: acc.result.attention?.sound_pack ?? "agence.default",
