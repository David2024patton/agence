...
export async function createRuntimeLifecycle(input: LifecycleInput): Promise<Lifecycle> {
  return withRunSpan(
    "RunLifecycle.boot",
    {
      "agence.agent.name": input.agent,
      "agence.directory": input.directory,
      "agence.first": input.first,
      "agence.model.provider": input.model?.providerID,
      "agence.model.id": input.model?.modelID,
      "agence.model.variant": input.variant,
      ...
      try {
        ...
        const close = async (next: {
          ...
          return withRunSpan(
            "RunLifecycle.close",
            {
              "agence.show_exit": next.showExit,
