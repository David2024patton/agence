...
import { createOpencodeClient } from "@agence-ai/sdk/v2"
...
async function runInteractiveRuntime(input: RunRuntimeInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "agence.mode": input.resolveSession ? "local" : "attach",
      "agence.initial_input": !!input.initialInput,
      "agence.demo": input.demo,
    },
    async (span) => {
      ...
      setRunSpanAttributes(span, {
        "agence.directory": ctx.directory,
        "agence.resume": ctx.resume === true,
        "agence.agent.name": state.agent,
        "agence.model.provider": state.model?.providerID,
        "agence.model.id": state.model?.modelID,
        "agence.model.variant": state.activeVariant,
      ...
      const shell = await createRuntimeLifecycle({
        ...
        onModelSelect: async (model) => {
          ...
          setRunSpanAttributes(span, {
            "agence.model.provider": model.providerID,
            "agence.model.id": model.modelID,
            "agence.model.variant": state.activeVariant,
          })
          return {
          ...
          run: async (prompt, signal) => {
            ...
            return withRunSpan(
              "RunInteractive.turn",
              {
                ...
                "agence.model.id": state.model?.modelID,
                "agence.model.variant": state.activeVariant,
                "agence.prompt.chars": prompt.text.length,
                "agence.prompt.parts": prompt.parts.length,
                "agence.prompt.include_files": includeFiles,
                "agence.prompt.file_parts": includeFiles ? input.files.length : 0,
                "session.id": state.sessionID || undefined,
              },
              async (span) => {
                try {
...
export async function runInteractiveLocalMode(input: RunLocalInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "agence.directory": input.directory,
      "agence.initial_input": !!input.initialInput,
      "agence.demo": input.demo,
    },
    async () => {
      const sdk = createOpencodeClient({
        baseUrl: "http://agence.internal",
        fetch: input.fetch,
