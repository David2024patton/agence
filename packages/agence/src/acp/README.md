...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for agence.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal agence sessions
...
agence acp --cwd /path/to/project
```
...
```bash
AGENCE_ENABLE_QUESTION_TOOL=1 agence acp
```
...
- **Terminal Integration**: Full terminal support via agence's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | agence acp
```
...
ACP sessions map cleanly to agence's internal session model:
