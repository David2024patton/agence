<!--
  ...
  packages/agence/src/skill/index.ts (see CUSTOMIZE_AGENCE_SKILL_NAME
  and CUSTOMIZE_AGENCE_SKILL_DESCRIPTION). The body below becomes the
  skill's content.
-->

# Customizing agence

agence validates its own config strictly and refuses to start when a field
...
**<https://github.com/David2024patton/agence/config.json>**
...
agence's skill loader scans for `**/SKILL.md` inside skill directories. The
...
agence ships with `build`, `plan`, `general`, `explore`, plus optionally
`scout` (gated on `AGENCE_EXPERIMENTAL_SCOUT`). Hidden internal agents:
...
```ts
import type { Plugin } from "@agence-ai/plugin"
...
agence evaluates the LAST matching rule, so put broad rules first and narrow
rules last.
...
- `AGENCE_CONFIG_CONTENT='{"$schema":"https://github.com/David2024patton/agence/config.json"}'`:
  inject inline JSON as a final local-scope merge.
- `AGENCE_DISABLE_DEFAULT_PLUGINS=1`: skip default plugins.
- `AGENCE_PURE=1`: skip external plugins entirely.
- `AGENCE_DISABLE_EXTERNAL_SKILLS=1`,
  `AGENCE_DISABLE_CLAUDE_CODE_SKILLS=1`: skip the external skill scans under
  `~/.claude/` and `~/.agents/`.
...
- If the user's existing config is malformed, point them at the env-var escape
  ...
  session.
- After saving any config change, remind the user to quit and restart agence
