...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for agence serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(AgenceHttpApi)
...
export const PublicApi = AgenceHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "agence",
    version: "1.0.0",
