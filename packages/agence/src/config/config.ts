...
import { InstallationLocal, InstallationVersion } from "@agence-ai/core/installation/version"
...
import { NonNegativeInt, PositiveInt, type DeepMutable } from "@agence-ai/core/schema"
...
function normalizeLoadedConfig(data: unknown, source: string) {
  ...
  delete copy.tui
  log.warn("tui keys in agence config are deprecated; move them to tui.json", { path: source })
  return copy
}
...
export const Info = Schema.Struct({
  ...
  server: Schema.optional(ConfigServer.Server).annotate({
    description: "Server configuration for agence serve and web commands",
  }),
  command: Schema.optional(Schema.Record(Schema.String, ConfigCommand.Info)).annotate({
    description: "Command configuration, see https://github.com/David2024patton/agence/docs/commands",
  }),
  ...
  ).annotate({ description: "Agent configuration, see https://github.com/David2024patton/agence/docs/agents" }),
...
// Uses the shared `DeepMutable` from `@agence-ai/core/schema`. See the definition
...
export class Service extends Context.Service<Service, Interface>()("@agence/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["agence.jsonc", "agence.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://github.com/David2024patton/agence/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.AGENCE_CONFIG && !Flag.AGENCE_CONFIG_DIR && !Flag.AGENCE_CONFIG_CONTENT) {
        const file = globalConfigFile()
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "https://github.com/David2024patton/agence/config.json" }, null, 2))
            .pipe(Effect.catch(() => Effect.void))
        }
      }
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "config.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "agence.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "agence.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "AGENCE_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/agence`
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://github.com/David2024patton/agence/config.json"
        ...
        if (Flag.AGENCE_CONFIG) {
          yield* merge(Flag.AGENCE_CONFIG, yield* loadFile(Flag.AGENCE_CONFIG, authEnv))
          log.debug("loaded custom config", { path: Flag.AGENCE_CONFIG })
        }

        if (!Flag.AGENCE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("agence", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.AGENCE_CONFIG_DIR) {
          log.debug("loading config from AGENCE_CONFIG_DIR", { path: Flag.AGENCE_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".agence") || dir === Flag.AGENCE_CONFIG_DIR) {
            for (const file of ["agence.json", "agence.jsonc"]) {
        ...
        if (process.env.AGENCE_CONFIG_CONTENT) {
          const source = "AGENCE_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.AGENCE_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          log.debug("loaded custom config from AGENCE_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["AGENCE_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("AGENCE_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.AGENCE_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.AGENCE_PERMISSION))
          } catch (err) {
            log.warn("AGENCE_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.AGENCE_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.AGENCE_DISABLE_PRUNE) {
