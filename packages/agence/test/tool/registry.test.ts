...
describe("tool.registry", () => {
  ...
  it.instance("loads tools from .opencode/tool (singular)", () =>
    Effect.gen(function* () {
      ...
      const agence = path.join(test.directory, ".opencode")
      const tool = path.join(agence, "tool")
  ...
  // came in through `plugin.list()` — `oh-my-agence` was registering a tool
  ...
  it.instance("loads tools from .opencode/tools (plural)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const agence = path.join(test.directory, ".opencode")
      const tools = path.join(agence, "tools")
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const agence = path.join(test.directory, ".opencode")
        const customTools = path.join(agence, "tools")
        const plugin = path.join(agence, "node_modules", "@agence-ai", "plugin")
        yield* Effect.promise(() => fs.mkdir(path.join(plugin, "dist"), { recursive: true }))
        yield* Effect.promise(() => fs.mkdir(customTools, { recursive: true }))
        yield* Effect.promise(() =>
          fs.cp(path.dirname(fileURLToPath(import.meta.resolve("zod"))), path.join(agence, "node_modules", "zod"), {
            dereference: true,
            recursive: true,
          }),
        )
        yield* Effect.promise(() =>
          Bun.write(
            ...
            JSON.stringify({ name: "@agence-ai/plugin", type: "module", exports: { ".": "./dist/index.js" } }),
          ),
        )
        yield* Effect.promise(() =>
          Bun.write(
        ...
        yield* Effect.promise(() =>
          Bun.write(
            ...
            [
              'import { tool } from "@agence-ai/plugin"',
              "export default tool({",
  ...
  it.instance("loads tools with external dependencies without crashing", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(agence, "package.json"),
          JSON.stringify({
            name: "custom-tools",
            dependencies: {
              "@agence-ai/plugin": "^0.0.0",
              cowsay: "^1.6.0",
            },
          }),
        ),
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(agence, "package-lock.json"),
          JSON.stringify({
            name: "custom-tools",
            lockfileVersion: 3,
            packages: {
      ...
      const cowsay = path.join(agence, "node_modules", "cowsay")
