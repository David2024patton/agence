...
function remoteConfigClient(input: {
  ...
  return HttpClient.make((request) => {
    if (request.url.includes(".well-known/agence")) {
...
const managedConfigDir = process.env.AGENCE_TEST_MANAGED_CONFIG_DIR!
...
test("does not create global config when AGENCE_CONFIG_DIR is set", async () => {
  await using tmp = await tmpdir()
  await using custom = await tmpdir()
  const prevConfig = Global.Path.config
  const prevEnv = process.env.AGENCE_CONFIG_DIR
  ;(Global.Path as { config: string }).config = tmp.path
  process.env.AGENCE_CONFIG_DIR = custom.path
  await clear(true)
  ...
  try {
    ...
    else process.env.AGENCE_CONFIG_DIR = prevEnv
    await clear(true)
  }
})
...
it.instance("updates config and preserves empty shell sentinel", () =>
  Effect.gen(function* () {
    ...
    yield* writeConfigEffect(
      test.directory,
      { $schema: "https://github.com/David2024patton/agence/config.json", shell: "bash" },
      "config.json",
    )
...
it.instance("ignores legacy tui keys in agence config", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["AGENCE_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { agence: { options: { apiKey: "{env:AGENCE_CONSOLE_TOKEN}" } } },
        }),
      ),
    ...
    await provideTmpdirInstance(() =>
      Config.Service.use((svc) =>
        Effect.gen(function* () {
          ...
          expect(config.provider?.["agence"]?.options?.apiKey).toBe("st_test_token")
        }),
      ),
    ).pipe(Effect.scoped, Effect.provide(layer), Effect.runPromise)
  } finally {
    if (originalControlToken !== undefined) {
      process.env["AGENCE_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["AGENCE_CONSOLE_TOKEN"]
    }
  }
})
...
test("does not try to install dependencies in read-only AGENCE_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.AGENCE_CONFIG_DIR
  process.env.AGENCE_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.AGENCE_CONFIG_DIR
    else process.env.AGENCE_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable AGENCE_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
// Note: preload.ts sets AGENCE_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(fetchedUrl).toBe(`${server.url.origin}/.well-known/agence`)
            expect(config.mcp?.jira?.enabled).toBe(true)
          }),
        ),
      { git: true },
...
describe("AGENCE_DISABLE_PROJECT_CONFIG", () => {
  it.instance(
  ...
  it.instance(
    ...
    () =>
      withProcessEnvs(
        { AGENCE_CONFIG_DIR: undefined, AGENCE_DISABLE_PROJECT_CONFIG: "true" },
        Effect.gen(function* () {
          const test = yield* TestInstance
  ...
  it.instance(
    "AGENCE_CONFIG_DIR still works when flag is set",
    () =>
      Effect.gen(function* () {
        const configDir = yield* tmpdirScoped({ config: { model: "configdir/model" } })
        yield* withProcessEnvs(
          { AGENCE_DISABLE_PROJECT_CONFIG: "true", AGENCE_CONFIG_DIR: configDir },
          Effect.gen(function* () {
            const config = yield* Config.use.get()
...
// Regression for #28206: malformed AGENCE_PERMISSION JSON used to crash
...
describe("AGENCE_PERMISSION env var", () => {
  it.instance("does not crash when AGENCE_PERMISSION contains invalid JSON", () =>
    withProcessEnv(
      "AGENCE_PERMISSION",
      "{invalid",
      Effect.gen(function* () {
...
describe("AGENCE_CONFIG_CONTENT token substitution", () => {
  it.instance("substitutes {env:} tokens in AGENCE_CONFIG_CONTENT", () =>
    withProcessEnv(
      "TEST_CONFIG_VAR",
      "test_api_key_12345",
      withProcessEnv(
        "AGENCE_CONFIG_CONTENT",
        JSON.stringify({
          $schema: "https://github.com/David2024patton/agence/config.json",
  ...
  it.instance("substitutes {file:} tokens in AGENCE_CONFIG_CONTENT", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
...
test("parseManagedPlist handles empty config", async () => {
  const config = ConfigParse.schema(
    Config.Info,
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(JSON.stringify({ $schema: "https://github.com/David2024patton/agence/config.json" })),
      "test:mobileconfig",
    ),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://github.com/David2024patton/agence/config.json")
})
