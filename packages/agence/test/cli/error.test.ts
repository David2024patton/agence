...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      ...
      },
      {
        ...
        data: { path: "/tmp/opencode.jsonc", dir: ".opencode", suggestion: "agence" },
        expected:
          'Directory ".opencode" in /tmp/opencode.jsonc is not valid. Rename the directory to "agence" or remove it. This is a common typo.',
      },
      {
  ...
  test("formats legacy and tagged provider model errors the same way", () => {
    ...
    const expected = [
      ...
      "Try: `agence models` to list available models",
