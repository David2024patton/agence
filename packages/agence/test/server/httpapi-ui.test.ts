...
const testStateLayer = Layer.effectDiscard(
  Effect.gen(function* () {
    const original = {
      ...
      envPassword: process.env.AGENCE_SERVER_PASSWORD,
      envUsername: process.env.AGENCE_SERVER_USERNAME,
    }
    ...
    yield* Effect.addFinalizer(() =>
      Effect.sync(() => {
        ...
        restoreEnv("AGENCE_SERVER_PASSWORD", original.envPassword)
        restoreEnv("AGENCE_SERVER_USERNAME", original.envUsername)
      }),
    )
  }),
)
      ...
      Layer.provide(
        ConfigProvider.layer(
          ConfigProvider.fromUnknown({
            AGENCE_SERVER_PASSWORD: input?.password,
            AGENCE_SERVER_USERNAME: input?.username,
          }),
        ),
      ),
    ),
...
describe("HttpApi UI fallback", () => {
  it.live("serves the web UI through the HTTP API app", () =>
    Effect.gen(function* () {
      ...
      const response = yield* uiApp({
        ...
        client: httpClient(
          new Response("<html>agence</html>", { headers: { "content-type": "text/html" } }),
          (request) => {
            proxiedUrl = request.url
          },
        ),
      ...
      expect(yield* responseText(response)).toBe("<html>agence</html>")
  ...
  it.live("accepts auth token for the web UI", () =>
    Effect.gen(function* () {
      const response = yield* uiApp({
        ...
        username: "agence",
        disableEmbeddedWebUi: true,
        client: httpClient(new Response("<html>agence</html>", { headers: { "content-type": "text/html" } })),
      }).request(`/?auth_token=${btoa("agence:secret")}`)
  ...
  it.live("accepts basic auth for the web UI", () =>
    Effect.gen(function* () {
      const response = yield* uiApp({
        ...
        headers: { authorization: `Basic ${btoa("agence:secret")}` },
      })
  ...
  it.live("allows web UI preflight without auth", () =>
    Effect.gen(function* () {
      const response = yield* app({ password: "secret", username: "agence" }).request("/", {
        method: "OPTIONS",
        headers: {
