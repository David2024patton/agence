// Subprocess test harness for the agence CLI. Spawns the real binary against
...
//
// Configuration flows through agence's built-in test affordances:
//   - AGENCE_CONFIG_CONTENT      : provider config inline, no files to find
//   - AGENCE_TEST_HOME           : pins os.homedir() → tmpdir
//   - AGENCE_DISABLE_PROJECT_CONFIG : skip walking up for opencode.json
//   - AGENCE_PURE                : skip external plugin discovery + install
//   - AGENCE_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `agence.run` is fully wired. The shape supports adding more
// builders (`agence.serve(opts)`, `agence.acp(opts)`, `agence.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of OpencodeCli.
import { test, type TestOptions } from "bun:test"
import { AppFileSystem } from "@agence-ai/core/filesystem"
import { AppProcess } from "@agence-ai/core/process"
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    AGENCE_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    AGENCE_CONFIG_CONTENT: configJson,
    AGENCE_DISABLE_PROJECT_CONFIG: "1",
    AGENCE_PURE: "1",
    AGENCE_DISABLE_AUTOUPDATE: "1",
    AGENCE_DISABLE_AUTOCOMPACT: "1",
    AGENCE_DISABLE_MODELS_FETCH: "1",
    AGENCE_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `agence run`. New flags should
...
// `agence serve` is a long-lived process — it never exits on its own.
...
// `agence acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type OpencodeCli = {
  ...
  // Spawn `agence serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `agence acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("agence.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const serve = Effect.fn("agence.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "agence server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `agence serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("agence.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const agence: OpencodeCli = { run, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, agence })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `agence.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
...
function expectExit(result: RunResult, expected: number, label = "agence") {
...
// every call site. Use this for any test that needs the agence CLI fixture.
//
...
// resources (e.g. `agence.serve`) without an extra `Effect.scoped` wrapper —
