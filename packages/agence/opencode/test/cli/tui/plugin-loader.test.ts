...
import type { TuiAttentionSoundPack } from "@agence-ai/plugin/tui"
...
`,
  ...
  process.env.AGENCE_PLUGIN_META_FILE = path.join(tmp.path, "plugin-meta.json")
  const config = createTuiResolvedConfig({
    plugin: [
...
`,
    ...
    const list = await Filesystem.readJson<Record<string, { themes?: Record<string, { dest: string }> }>>(
      process.env.AGENCE_PLUGIN_META_FILE!,
    )
