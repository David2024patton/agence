...
// terminal sizes. The default agence tui command is excluded —
// `agence --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `agence --help` prints today;
...
// top-level help on `--help` and exits 1; not a real agence command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("agence CLI help-text snapshots", () => {
    ...
    ({ agence }) =>
      Effect.gen(function* () {
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* agence.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`agence ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`agence ${argv.join(" ")} --help`)
        }
