...
describe("agence read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // AGENCE_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ agence }) =>
      Effect.gen(function* () {
        const r = yield* agence.spawn(["mcp", "list"])
        agence.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ agence }) =>
      Effect.gen(function* () {
        const r = yield* agence.spawn(["providers", "list"])
        agence.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ agence }) =>
      Effect.gen(function* () {
        const r = yield* agence.spawn(["models"])
        agence.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ agence }) =>
      Effect.gen(function* () {
        const r = yield* agence.spawn(["agent", "list"])
        agence.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh AGENCE_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ agence }) =>
      Effect.gen(function* () {
        const r = yield* agence.spawn(["session", "list"])
        agence.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ agence }) =>
      Effect.gen(function* () {
        const r = yield* agence.spawn(["stats"])
        agence.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under AGENCE_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ agence }) =>
      Effect.gen(function* () {
        const r = yield* agence.spawn(["db", "path"])
        agence.expectExit(r, 0, "db path")
