...
describe("websearch provider", () => {
  ...
  test("supports an operational override", () => {
    const original = process.env.AGENCE_WEBSEARCH_PROVIDER

    try {
      process.env.AGENCE_WEBSEARCH_PROVIDER = "parallel"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("parallel")

      process.env.AGENCE_WEBSEARCH_PROVIDER = "exa"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("exa")
    } finally {
      if (original === undefined) delete process.env.AGENCE_WEBSEARCH_PROVIDER
      else process.env.AGENCE_WEBSEARCH_PROVIDER = original
    }
  })
  ...
  test("is only enabled for agence or explicit websearch provider flags", () => {
