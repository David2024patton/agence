...
import { HttpRecorder, Redactor } from "@agence-ai/http-recorder"
...
import { LLMEvent, LLMResponse } from "@agence-ai/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor } from "@agence-ai/llm/route"
import { Env } from "@/env"
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.AGENCE_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["agence", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("AGENCE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("AGENCE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["agence", "native", "oauth", "tool-loop"],
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["agence", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.AGENCE_RECORD_CONSOLE_TOKEN && process.env.AGENCE_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderID.opencode,
        name: "Agence Zen",
        env: ["AGENCE_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.AGENCE_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.AGENCE_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.AGENCE_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["agence", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("AGENCE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("AGENCE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("AGENCE_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set AGENCE_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the agence LLM stack remain real.
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      ...
      JSON.stringify({ $schema: "https://github.com/David2024patton/agence/config.json", ...scenario.config(model) }),
    ),
  )
