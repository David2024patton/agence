...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.AGENCE_TEST ?? "")'), {
      env: {
        AGENCE_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "AGENCE_TEST_SHELL"], {
      shell: true,
      env: {
        AGENCE_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("AGENCE_TEST_SHELL=ok")
  })
