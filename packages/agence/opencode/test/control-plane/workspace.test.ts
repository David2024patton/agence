...
const originalEnv = {
  AGENCE_AUTH_CONTENT: process.env.AGENCE_AUTH_CONTENT,
  AGENCE_EXPERIMENTAL_WORKSPACES: process.env.AGENCE_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  Database.close()
  restoreEnv()
  process.env.AGENCE_EXPERIMENTAL_WORKSPACES = "true"
})
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.AGENCE_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        ...
        expect(JSON.parse(recorded.calls.create[0].env.AGENCE_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.AGENCE_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.AGENCE_EXPERIMENTAL_WORKSPACES).toBe("true")
