...
const auth = { username: "agence", password: "listen-secret" }
...
afterEach(async () => {
  ...
  if (original.envPassword === undefined) delete process.env.AGENCE_SERVER_PASSWORD
  else process.env.AGENCE_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.AGENCE_SERVER_USERNAME
  else process.env.AGENCE_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.AGENCE_SERVER_PASSWORD = auth.password
  Flag.AGENCE_SERVER_USERNAME = auth.username
  process.env.AGENCE_SERVER_PASSWORD = auth.password
  process.env.AGENCE_SERVER_USERNAME = auth.username
...
async function startNoAuthListener() {
  ...
  Flag.AGENCE_SERVER_USERNAME = auth.username
  delete process.env.AGENCE_SERVER_PASSWORD
  process.env.AGENCE_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}
