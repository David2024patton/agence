 ...
 *
 * The script intentionally isolates `AGENCE_DB` before importing modules that touch
...
void (await import("@agence-ai/core/util/log")).init({ print: false })
...
const scenarios: Scenario[] = [
  ...
  http.protected
    ...
    .at((ctx) => ({
      path: `/experimental/tool?${new URLSearchParams({ provider: "agence", model: "test" })}`,
      headers: ctx.headers(),
    }))
