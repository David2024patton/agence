...
import { tool as nativeTool, ToolFailure, type JsonSchema, type LLMEvent } from "@agence-ai/llm"
import type { LLMClientShape } from "@agence-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("agence"))
    return { type: "unsupported", reason: "provider is not openai, agence, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @agence-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      ...
      // the @agence-ai/llm tool call back into the AI SDK Tool.execute shape.
      nativeTool({
