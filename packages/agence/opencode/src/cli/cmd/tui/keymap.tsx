...
export const AGENCE_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const AGENCE_MODE_KEY = "agence.mode"
...
export function createOpencodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(AGENCE_MODE_KEY, AGENCE_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(AGENCE_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(AGENCE_MODE_KEY, stack.at(-1)?.mode ?? AGENCE_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? AGENCE_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(AGENCE_MODE_KEY, undefined)
