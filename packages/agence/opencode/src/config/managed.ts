...
const MANAGED_PLIST_DOMAIN = "ai.agence.managed"

// Keys injected by macOS/MDM into the managed plist that are not Agence config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/agence"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "agence")
    default:
      return "/etc/agence"
  }
}
...
export function managedConfigDir() {
  return process.env.AGENCE_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
