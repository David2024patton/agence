...
Delete `packages/agence/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/agence/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/agence/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/agence`
- TUI plugin loader tests in `packages/agence` if runtime plugin API wiring changed
