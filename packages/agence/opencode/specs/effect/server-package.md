...
Practical reference for a future `packages/server` split after the agence
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/agence`.
...
Until enough shared service code lives outside `packages/agence`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/agence`

It should not import `packages/agence` services while `packages/agence`
...
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/agence` implementation details.
