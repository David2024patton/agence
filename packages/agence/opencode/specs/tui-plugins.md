...
- Author package entrypoint is `@agence-ai/plugin/tui`.
...
```json
{
  "$schema": "https://github.com/David2024patton/agence/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
...
- Import types from `@agence-ai/plugin/tui`.
- `@agence-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@agence-ai/plugin/tui"
...
- The value is a semver range checked against the running Agence version.
...
- `agence plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `agence plug <module>`.
...
When `plugin` entries exist in a writable `.opencode` dir or `AGENCE_CONFIG_DIR`, Agence installs `@agence-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@agence-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus Agence's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
Agence registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, Agence pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@agence-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"agence"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `agence.default`.
...
- `--pure` / `AGENCE_PURE` skips external TUI plugins only.
