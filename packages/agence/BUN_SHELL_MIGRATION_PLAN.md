...
Replace runtime Bun shell template-tag usage in `packages/agence/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/agence/src` is removed except:
