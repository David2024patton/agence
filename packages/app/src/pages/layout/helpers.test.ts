...
import { type Session } from "@agence-ai/sdk/v2/client"
import {
  childSessionOnPath,
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("agence://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("agence://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("agence://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("agence://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("agence://open-project")).toBeUndefined()
    expect(parseDeepLink("agence://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "agence://open-project?directory=/a",
      "agence://other?directory=/b",
      "agence://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("agence://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("agence://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("agence://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("agence://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "agence://new-session?directory=/a",
      "agence://open-project?directory=/b",
      "agence://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __AGENCE__: {
        deepLinks: ["agence://open-project?directory=/a"],
      },
    } as unknown as Window & { __AGENCE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["agence://open-project?directory=/a"])
