import { useFilteredList } from "@agence-ai/ui/hooks"
import { useSpring } from "@agence-ai/ui/motion-spring"
import {
  createEffect,
  on,
  Component,
...
import { DockShellForm, DockTray } from "@agence-ai/ui/dock-surface"
...
import { ImagePreview } from "@agence-ai/ui/image-preview"
...
export const PromptInput: Component<PromptInputProps> = (props) => {
  ...
  const [chatMode, setChatMode] = persisted(
    Persist.global("prompt-chat-mode", ["prompt-chat-mode.v1"]),
    createStore<{ mode: "build" | "plan" | "research" }>({ mode: "build" }),
  )
    ...
    <Show when={!providersLoading()}>
      <div class="flex items-center gap-2 min-w-0">
        ...
        <Show when={store.mode !== "shell"}>
          ...
          <Select
            size="normal"
            options={["build", "plan", "research"] as const}
            current={chatMode.mode}
            label={(x) => (x === "build" ? "Build" : x === "plan" ? "Plan" : "Research")}
            onSelect={(value) => {
              if (!value) return
