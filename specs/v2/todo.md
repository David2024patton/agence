...
The agence server has moved to the Effect HttpApi backend. Remaining work is
...
- plugins get global "agence" instance like in that post i showed
- agence instance has stuff like `agence.session.prompt()` or
  `agence.tool.register({...})`
