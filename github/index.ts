...
import { createOpencodeClient } from "@agence-ai/sdk"
...
function createOpencode() {
  ...
  const proc = spawn(`agence`, [`serve`, `--hostname=${host}`, `--port=${port}`])
...
function assertPayloadKeyword() {
  ...
  if (!body.match(/(?:^|\s)(?:\/agence|\/oc)(?=$|\s)/)) {
    throw new Error("Comments must mention `/agence` or `/oc`")
  }
}
...
async function assertOpencodeConnected() {
  ...
  if (!connected) {
    throw new Error("Failed to connect to agence server")
  }
}
...
function useShareUrl() {
  return isMock() ? "https://dev.opencode.ai" : "https://github.com/David2024patton/agence"
}
...
async function getUserPrompt() {
  ...
  let prompt = (() => {
    const body = payload.comment.body.trim()
    if (body === "/agence" || body === "/oc") {
      if (reviewContext) {
        return `Review this code change and suggest improvements for the commented lines:\n\nFile: ${reviewContext.file}\nLines: ${reviewContext.line}\n\n${reviewContext.diffHunk}`
      }
      return "Summarize this thread"
    }
    if (body.includes("/agence") || body.includes("/oc")) {
      if (reviewContext) {
...
function footer(opts?: { image?: boolean }) {
  ...
  const shareUrl = shareId ? `[agence session](${useShareUrl()}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
