...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("packages/agence")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with agence...`)
  ...
  try {
    await $`agence run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  agence failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with agence")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with agence...")
  ...
  const prompt = [
    ...
    "Run `./script/build.ts --single` in `packages/agence`.",
