...
import { createOpencode } from "@agence-ai/sdk"
...
`)
  ...
  const agence = await createOpencode({ port: 0 })
  ...
  try {
    ...
    const session = await agence.client.session.create()
    const result = await agence.client.session
      .prompt({
